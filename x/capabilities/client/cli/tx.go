...
func GetTxCmd() *cobra.Command {
	txCmd := &cobra.Command{
		...
		Short:                      "ucapabilities module transactions",
...
func CmdRegister() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Register as an AI capabilities",
		Long:  "Register as an AI capabilities on the Axon network.\nExample: axond tx capabilities register \"coding,reasoning\" \"gpt-4\" 100000000000000000000aaxon",
...
func CmdAddStake() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Add stake to an existing AI capabilities",
		Long:  "Increase the stake of an already-registered AI capabilities.\nExample: axond tx capabilities add-stake 50000000000000000000aaxon",
...
func CmdDeregister() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "deregister",
		Short: "Request to deregister as an AI capabilities (7-day cooldown)",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func CmdUpdateucapabilities() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update [capabilities] [model]",
		Short: "Update capabilities capabilities and model",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := &types.MsgUpdateucapabilities{
				Sender:       clientCtx.GetFromAddress().String(),
				Capabilities: args[0],
				Model:        args[1],
			}
