...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy capabilities.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all capabilitiess and store for validator updates
...
// checkHeartbeatTimeouts collects offline capabilitiess first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	blockHeight := ctx.BlockHeight()

	type offlineucapabilities struct {
		capabilities types.ucapabilities
	}

	var toOffline []offlineucapabilities
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		if capabilities.Status == types.ucapabilitiesStatus_CAPABILITIES_STATUS_ONLINE &&
			blockHeight-capabilities.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineucapabilities{capabilities: capabilities})
		}
		return false
	})

	for _, o := range toOffline {
		o.capabilities.Status = types.ucapabilitiesStatus_CAPABILITIES_STATUS_OFFLINE
		k.Setucapabilities(ctx, o.capabilities)
		k.UpdateReputation(ctx, o.capabilities.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.capabilities.Address)

		k.Logger(ctx).Info("capabilities went offline",
			"address", o.capabilities.Address,
			"last_heartbeat", o.capabilities.LastHeartbeat,
...
const jailedFlagKeyPrefix = "ucapabilitiesJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// capabilitiess and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(capabilities.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + capabilities.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, capabilities.Address)
			k.UpdateReputation(ctx, capabilities.Address, types.ReputationLossSlashing)
