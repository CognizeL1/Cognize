...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible capabilitiess.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		if !k.isActiveValidatorAddress(ctx, capabilities.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, capabilities.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(capabilities.Address)
		if err != nil {
			return false
		}
...
// ucapabilitiess proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered capabilitiess (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		totalRep := k.GetTotalReputation(ctx, capabilities.Address)
		if totalRep <= 0 {
			return false
		}
		...
		capabilitiess = append(capabilitiess, repWeight{address: capabilities.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(capabilitiess) == 0 {
		return totalAmount
	}
