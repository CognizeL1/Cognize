...
// CheaterAnswerThreshold is the minimum number of capabilitiess submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI capabilities registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		if capabilities.Status == types.ucapabilitiesStatus_CAPABILITIES_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, capabilities.Address) &&
			!respondents[capabilities.Address] {
			k.SetAIBonus(ctx, capabilities.Address, 0)
		}
		return false
	})
...
// detectCheaters flags capabilitiess that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — capabilitiess copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := capabilities.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		capabilities.StakeAmount = capabilities.StakeAmount.Sub(slashCoin)
