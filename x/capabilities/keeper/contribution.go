...
const (
	...
	// Whitepaper §9.4: ucapabilities 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — capabilitiess with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — capabilitiess registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoreducapabilities struct {
		address string
		score   int64
		stake   *big.Int
	}

	var capabilitiess []scoreducapabilities
	totalScore := int64(0)
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		if capabilities.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-capabilities.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, capabilities)
		if score <= 0 {
			return false
		}

		stake := capabilities.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		capabilitiess = append(capabilitiess, scoreducapabilities{address: capabilities.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(capabilitiess) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range capabilitiess {
		share := new(big.Int).Mul(poolBig, big.NewInt(a.score))
		share.Div(share, big.NewInt(totalScore))

		maxPerucapabilities := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPerucapabilities) > 0 {
			share.Set(maxPerucapabilities)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("capabilitiess", fmt.Sprintf("%d", len(capabilitiess))),
	))
}

// calculateContributionScore computes a weighted score for an capabilities.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, capabilities types.ucapabilities) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, capabilities.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, capabilities.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, capabilities.Address)
	...
	if capabilities.Reputation > 70 {
		score += int64(capabilities.Reputation-70) * WeightHighReputation
	}

	// Online bonus
	if capabilities.Status == types.ucapabilitiesStatus_CAPABILITIES_STATUS_ONLINE {
		score += 5
	}
...
func contributionRewardCap(poolAmount, capabilitiesStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || capabilitiesStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || capabilitiesStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, capabilitiesStake)
