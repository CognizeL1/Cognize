...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if capabilities.BurnedAtRegister.Denom != "" && capabilities.BurnedAtRegister.IsPositive() {
		burnedAmount = capabilities.BurnedAtRegister
	} else {
	...
	if capabilities.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = capabilities.StakeAmount.Sub(burnedAmount)
	}

	if capabilities.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.Deleteucapabilities(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupucapabilitiesEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteucapabilitiesIdentity(ctx, address)
	}

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"capabilities_deregistered",
		sdk.NewAttribute("address", address),
		sdk.NewAttribute("module_held", fmt.Sprintf("%s", moduleHeld)),
	))

	k.Logger(ctx).Info("capabilities deregistered after cooldown", "address", address)
}

// cleanupucapabilitiesEpochData removes epoch-scoped data for a deregistered capabilities.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other capabilitiess
// whose address might be a raw substring of the key.
func (k Keeper) cleanupucapabilitiesEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
