...
func (m *mockPrivacyKeeper) DeleteucapabilitiesIdentity(_ sdk.Context, capabilitiesAddr string) {
	m.deleted = append(m.deleted, capabilitiesAddr)
}
...
func TestExecuteDeregisterDeletesPrivacyIdentity(t *testing.T) {
	...
	capabilities := types.ucapabilities{
		Address:          address,
		ucapabilitiesId:          address,
		Status:           types.ucapabilitiesStatus_CAPABILITIES_STATUS_SUSPENDED,
	...
	k.SetDeregisterRequest(ctx, capabilities.Address, ctx.BlockHeight()-types.DeregisterCooldownBlocks)

	k.executeDeregister(ctx, capabilities.Address, k.GetParams(ctx))

	if _, found := k.Getucapabilities(ctx, capabilities.Address); found {
		t.Fatal("capabilities should be deleted after deregister")
	}
	if k.HasDeregisterRequest(ctx, capabilities.Address) {
		t.Fatal("deregister request should be deleted after execution")
	}
	if len(privacyKeeper.deleted) != 1 || privacyKeeper.deleted[0] != capabilities.Address {
		t.Fatalf("expected privacy identity cleanup for %q, got %+v", capabilities.Address, privacyKeeper.deleted)
	}
}
