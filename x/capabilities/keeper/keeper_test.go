...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.ucapabilitiess) != 0 {
		t.Errorf("expected 0 capabilitiess in default genesis, got %d", len(gs.ucapabilitiess))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	capabilitiesKey := types.Keyucapabilities(addr)
	if string(capabilitiesKey) != types.ucapabilitiesKeyPrefix+addr {
		t.Errorf("Keyucapabilities mismatch: %s", capabilitiesKey)
	}
