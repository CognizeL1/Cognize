...
// ProcessL1Reputation is called at epoch end. It evaluates all capabilitiess and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		addr := capabilities.Address
		if k.shouldFreezeucapabilitiesReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old capabilities.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		if k.shouldFreezeucapabilitiesReputationDuringDeregister(ctx, capabilities.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, capabilities.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if capabilities.Reputation != legacy {
			capabilities.Reputation = legacy
...
// dual-layer L1/L2 score stores when an capabilities is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when capabilities goes offline.
...
// isEpochCheater checks if an capabilities was flagged as a cheater in the given epoch.
