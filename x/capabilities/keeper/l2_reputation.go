...
const (
	...
	DefaultL2BudgetPerucapabilitiesMillis int64 = 100     // 0.1 per capabilities
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporterucapabilities, found := k.Getucapabilities(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporterucapabilities.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetucapabilities, found := k.Getucapabilities(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetucapabilities.Status == types.ucapabilitiesStatus_CAPABILITIES_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	capabilitiesCount := int64(0)
	k.Iterateucapabilitiess(ctx, func(_ types.ucapabilities) bool {
		capabilitiesCount++
		return false
	})

	budgetPerucapabilities := l2BudgetPerucapabilitiesMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPerucapabilities * capabilitiesCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("capabilitiess_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPerucapabilitiesMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPerucapabilities, types.DefaultParams().L2BudgetPerucapabilities, DefaultL2BudgetPerucapabilitiesMillis)
}
