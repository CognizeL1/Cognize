...
// ComputeAllMiningPowers iterates over all capabilitiess and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		stake := capabilities.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, capabilities.Address)
		...
		if power.IsPositive() {
			raw[capabilities.Address] = power
			if power.GT(maxRaw) {
				maxRaw = power
			}
		}
