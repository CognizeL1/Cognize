...
func (k msgServer) AddStake(goCtx context.Context, msg *types.MsgAddStake) (*types.MsgAddStakeResponse, error) {
	...
	return k.Keeper.AddStakeToucapabilities(ctx, msg.Sender, msg.Stake, senderAddr)
}

func (k msgServer) Updateucapabilities(goCtx context.Context, msg *types.MsgUpdateucapabilities) (*types.MsgUpdateucapabilitiesResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	capabilities, found := k.Getucapabilities(ctx, msg.Sender)
	if !found {
	...
	if msg.Model != "" {
		capabilities.Model = msg.Model
	}

	k.Setucapabilities(ctx, capabilities)
	return &types.MsgUpdateucapabilitiesResponse{}, nil
}
...
func (k msgServer) Heartbeat(goCtx context.Context, msg *types.MsgHeartbeat) (*types.MsgHeartbeatResponse, error) {
	...
	if ctx.BlockHeight()-capabilities.LastHeartbeat < params.HeartbeatInterval {
		return nil, types.ErrHeartbeatTooFrequent
	}

	capabilities.LastHeartbeat = ctx.BlockHeight()
	capabilities.Status = types.ucapabilitiesStatus_CAPABILITIES_STATUS_ONLINE
	k.Setucapabilities(ctx, capabilities)
...
func (k msgServer) Deregister(goCtx context.Context, msg *types.MsgDeregister) (*types.MsgDeregisterResponse, error) {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"capabilities_deregister_requested",
		sdk.NewAttribute("address", msg.Sender),
...
func (k msgServer) ReduceStake(goCtx context.Context, msg *types.MsgReduceStake) (*types.MsgReduceStakeResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := k.Keeper.ReduceStakeFromucapabilities(ctx, msg.Sender, msg.Amount); err != nil {
		return nil, err
	}
...
func (k msgServer) RevealAIChallengeResponse(goCtx context.Context, msg *types.MsgRevealAIChallengeResponse) (*types.MsgRevealAIChallengeResponseResponse, error) {
	...
	if _, found := k.Getucapabilities(ctx, msg.Sender); !found {
		return nil, types.ErrucapabilitiesNotFound
	}
	if k.HasDeregisterRequest(ctx, msg.Sender) {
...
func generateucapabilitiesID(address string, blockHeight int64) string {
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s:%d", address, blockHeight)))
	return fmt.Sprintf("capabilities-%s", hex.EncodeToString(hash[:8]))
}
