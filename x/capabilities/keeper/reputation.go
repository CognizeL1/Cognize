...
// IncrementEpochActivity increments the transaction count for an capabilities in the current epoch.
...
// GetEpochActivity returns the transaction count for an capabilities in a given epoch.
...
// ProcessEpochReputation updates reputation for all capabilitiess at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		if capabilities.Status == types.ucapabilitiesStatus_CAPABILITIES_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: capabilities.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updateducapabilities, found := k.Getucapabilities(ctx, u.address)
		if found && updateducapabilities.Reputation == 0 {
			k.handleZeroReputation(ctx, updateducapabilities, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the capabilities.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, capabilities types.ucapabilities, params types.Params) {
	var burnedAtRegister sdk.Coin
	if capabilities.BurnedAtRegister.Denom != "" && capabilities.BurnedAtRegister.IsPositive() {
		burnedAtRegister = capabilities.BurnedAtRegister
	} else {
		// Fallback for legacy capabilitiess without snapshot
	...
	if capabilities.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = capabilities.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", capabilities.Address, "error", err)
		} else {
			burned = true
		}
	}

	capabilities.Status = types.ucapabilitiesStatus_CAPABILITIES_STATUS_SUSPENDED
	if burned || !remaining.IsPositive() {
		capabilities.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.Setucapabilities(ctx, capabilities)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"capabilities_stake_burned",
		sdk.NewAttribute("address", capabilities.Address),
