...
// DistributeEpochRewards distributes contribution rewards to online capabilitiess
...
func (k Keeper) DistributeEpochRewards(ctx sdk.Context, epoch uint64) {
	...
	type weighteducapabilities struct {
		address string
		weight  *big.Int
	}

	var capabilitiess []weighteducapabilities
	...
	k.Iterateucapabilitiess(ctx, func(capabilities types.ucapabilities) bool {
		...
		stakeAmount := capabilities.StakeAmount.Amount.BigInt()

		repBonus := reputationBonusPercent(capabilities.Reputation)
		aiBonus := k.GetAIBonus(ctx, capabilities.Address)
		...
		capabilitiess = append(capabilitiess, weighteducapabilities{
	...
	if totalWeight.Sign() <= 0 || len(capabilitiess) == 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("capabilitiess_count", fmt.Sprintf("%d", len(capabilitiess))),
	))
}
