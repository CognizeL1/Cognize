...
func TestSubmitL2ReportUsesFullWeightOnlyForIndexedEvidence(t *testing.T) {
	...
	setTestucapabilities(k, ctx, "reporter", 1)
	setTestucapabilities(k, ctx, "target", 1)
...
func TestSubmitL2ReportFallsBackWhenEvidenceMissingOrInvalid(t *testing.T) {
	...
	setTestucapabilities(k, ctx, "reporter1", 1)
	setTestucapabilities(k, ctx, "target1", 1)
	...
	setTestucapabilities(k, ctx, "reporter2", 1)
	setTestucapabilities(k, ctx, "target2", 1)
...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "capabilities1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "capabilities1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "capabilities1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "capabilities1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "capabilities1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "capabilities1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "capabilities1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "capabilities1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "capabilities1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "capabilities1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("capabilities1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("capabilities1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("capabilities1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("capabilities1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("capabilities1", 2)) || !store.Has(dailyRegisterKey("capabilities1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("capabilities1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("capabilities1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 capabilitiess × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "capabilities" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("capabilities0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, capabilities.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, capabilities.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, capabilities.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, capabilities)
	if score != 801105 {
...
func TestEvaluateEpochChallengesScoresCorrectlyBeforeUpgrade(t *testing.T) {
	...
	// Register the validator as capabilities
	setTestucapabilities(k, ctx, wrongResp.ValidatorAddress, 1)
...
func TestHistoricalReplayConsistency(t *testing.T) {
	...
	setTestucapabilities(k, ctx, resp1.ValidatorAddress, 1)
	...
	setTestucapabilities(k, ctx, resp2.ValidatorAddress, 1)
