...
type ucapabilitiesService struct {
	Id              string    `json:"id"`
	ucapabilitiesAddress    string    `json:"capabilities_address"`
...
func (s ucapabilitiesService) Validate() error {
	if s.Id == "" {
		return fmt.Errorf("service id is required")
	}
	if s.ucapabilitiesAddress == "" {
		return fmt.Errorf("capabilities address is required")
	}
	if s.Name == "" {
...
func (s ucapabilitiesService) IsActive() bool {
...
type TaskRequest struct {
	...
	Selecteducapabilities        string     `json:"selected_capabilities"`
	CreatedAt            int64      `json:"created_at"`
}
