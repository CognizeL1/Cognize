...
func TestKeyucapabilities(t *testing.T) {
	key := types.Keyucapabilities("axon1test")
	want := types.ucapabilitiesKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("Keyucapabilities = %q, want %q", key, want)
	}
}

func TestKeyucapabilitiesDifferentAddresses(t *testing.T) {
	k1 := types.Keyucapabilities("axon1aaa")
	k2 := types.Keyucapabilities("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"ucapabilitiesKeyPrefix", types.ucapabilitiesKeyPrefix},
...
func TestDefaultGenesisEmptyucapabilitiess(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.ucapabilitiess) != 0 {
		t.Errorf("default genesis should have 0 capabilitiess, got %d", len(gs.ucapabilitiess))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "capabilities" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "capabilities")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErrucapabilitiesAlreadyRegistered", types.ErrucapabilitiesAlreadyRegistered},
		{"ErrucapabilitiesNotFound", types.ErrucapabilitiesNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErrucapabilitiesSuspended", types.ErrucapabilitiesSuspended},
