...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy channels.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all channelss and store for validator updates
...
// checkHeartbeatTimeouts collects offline channelss first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	blockHeight := ctx.BlockHeight()

	type offlineChannels struct {
		channels types.Channels
	}

	var toOffline []offlineChannels

	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		if channels.Status == types.ChannelsStatus_CHANNELS_STATUS_ONLINE &&
			blockHeight-channels.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineChannels{channels: channels})
		}
		return false
	})

	for _, o := range toOffline {
		o.channels.Status = types.ChannelsStatus_CHANNELS_STATUS_OFFLINE
		k.SetChannels(ctx, o.channels)
		k.UpdateReputation(ctx, o.channels.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.channels.Address)

		k.Logger(ctx).Info("channels went offline",
			"address", o.channels.Address,
			"last_heartbeat", o.channels.LastHeartbeat,
...
const jailedFlagKeyPrefix = "ChannelsJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// channelss and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(channels.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + channels.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, channels.Address)
			k.UpdateReputation(ctx, channels.Address, types.ReputationLossSlashing)
