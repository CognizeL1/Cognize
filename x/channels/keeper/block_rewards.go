...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible channelss.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		if !k.isActiveValidatorAddress(ctx, channels.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, channels.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(channels.Address)
		if err != nil {
			return false
		}
...
// Channelss proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered channelss (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		totalRep := k.GetTotalReputation(ctx, channels.Address)
		if totalRep <= 0 {
			return false
		}
		...
		channelss = append(channelss, repWeight{address: channels.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(channelss) == 0 {
		return totalAmount
	}
