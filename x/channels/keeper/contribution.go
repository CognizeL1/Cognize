...
const (
	...
	// Whitepaper §9.4: Channels 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — channelss with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — channelss registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoredChannels struct {
		address string
		score   int64
		stake   *big.Int
	}

	var channelss []scoredChannels
	totalScore := int64(0)
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		if channels.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-channels.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, channels)
		if score <= 0 {
			return false
		}

		stake := channels.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		channelss = append(channelss, scoredChannels{address: channels.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(channelss) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range channelss {
		share := new(big.Int).Mul(poolBig, big.NewInt(a.score))
		share.Div(share, big.NewInt(totalScore))

		maxPerChannels := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPerChannels) > 0 {
			share.Set(maxPerChannels)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("channelss", fmt.Sprintf("%d", len(channelss))),
	))
}

// calculateContributionScore computes a weighted score for an channels.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, channels types.Channels) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, channels.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, channels.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, channels.Address)
	...
	if channels.Reputation > 70 {
		score += int64(channels.Reputation-70) * WeightHighReputation
	}
...
func contributionRewardCap(poolAmount, channelsStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || channelsStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || channelsStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, channelsStake)
