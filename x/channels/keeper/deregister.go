...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if channels.BurnedAtRegister.Denom != "" && channels.BurnedAtRegister.IsPositive() {
		burnedAmount = channels.BurnedAtRegister
	} else {
	...
	if channels.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = channels.StakeAmount.Sub(burnedAmount)
	}

	if channels.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.DeleteChannels(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupChannelsEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteChannelsIdentity(ctx, address)
	}

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"channels_deregistered",
		sdk.NewAttribute("address", address),
		sdk.NewAttribute("module_held", fmt.Sprintf("%s", moduleHeld)),
	))

	k.Logger(ctx).Info("channels deregistered after cooldown", "address", address)
}

// cleanupChannelsEpochData removes epoch-scoped data for a deregistered channels.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other channelss
// whose address might be a raw substring of the key.
func (k Keeper) cleanupChannelsEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
