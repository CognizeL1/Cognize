...
func (k Keeper) shouldFreezeChannelsReputationDuringDeregister(ctx sdk.Context, address string) bool {
...
func (k Keeper) GetChannels(ctx sdk.Context, address string) (types.Channels, bool) {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.KeyChannels(address))
	if bz == nil {
		return types.Channels{}, false
	}
	var channels types.Channels
	k.cdc.MustUnmarshal(bz, &channels)
	return channels, true
}

func (k Keeper) SetChannels(ctx sdk.Context, channels types.Channels) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshal(&channels)
	store.Set(types.KeyChannels(channels.Address), bz)
}

func (k Keeper) DeleteChannels(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(types.KeyChannels(address))
}

func (k Keeper) IterateChannelss(ctx sdk.Context, cb func(channels types.Channels) bool) {
	store := ctx.KVStore(k.storeKey)
	iterator := storetypes.KVStorePrefixIterator(store, []byte(types.ChannelsKeyPrefix))
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var channels types.Channels
		k.cdc.MustUnmarshal(iterator.Value(), &channels)
		if cb(channels) {
			break
		}
	}
...
func (k Keeper) GetAllChannelss(ctx sdk.Context) []types.Channels {
...
func (k Keeper) IsChannels(ctx sdk.Context, address string) bool {
	_, found := k.GetChannels(ctx, address)
	return found
}
...
// instead of from the channels's own address, avoiding double deduction.
func (k Keeper) RegisterFromPrecompile(ctx sdk.Context, msg *types.MsgRegister, fundsSource sdk.AccAddress) (*types.MsgRegisterResponse, error) {
	...
	if k.IsChannels(ctx, msg.Sender) {
		return nil, types.ErrChannelsAlreadyRegistered
	}
	...
	channels := types.Channels{
		Address:          msg.Sender,
		ChannelsId:          generateChannelsID(msg.Sender, ctx.BlockHeight()),
		Capabilities:     capabilities,
		Model:            msg.Model,
		Reputation:       params.InitialReputation,
		Status:           types.ChannelsStatus_CHANNELS_STATUS_ONLINE,
	...
	k.BootstrapLegacyReputation(ctx, channels.Address, channels.Reputation)
	k.IncrementDailyRegisterCount(ctx, msg.Sender)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"channels_registered",
		sdk.NewAttribute("address", msg.Sender),
		sdk.NewAttribute("channels_id", channels.ChannelsId),
		sdk.NewAttribute("stake", msg.Stake.String()),
		sdk.NewAttribute("burned", burnAmount.String()),
		sdk.NewAttribute("reputation", fmt.Sprintf("%d", channels.Reputation)),
	))

	return &types.MsgRegisterResponse{ChannelsId: channels.ChannelsId}, nil
}

func (k Keeper) AddStakeToChannels(ctx sdk.Context, sender string, stake sdk.Coin, fundsSource sdk.AccAddress) (*types.MsgAddStakeResponse, error) {
	channels, found := k.GetChannels(ctx, sender)
	if !found {
		return nil, types.ErrChannelsNotFound
	}
	if channels.Status == types.ChannelsStatus_CHANNELS_STATUS_SUSPENDED {
		return nil, types.ErrChannelsSuspended
	}
	...
	channels.StakeAmount = channels.StakeAmount.Add(stake)
	k.SetChannels(ctx, channels)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"channels_stake_added",
		sdk.NewAttribute("address", sender),
		sdk.NewAttribute("amount", stake.String()),
		sdk.NewAttribute("total_stake", channels.StakeAmount.String()),
	))

	return &types.MsgAddStakeResponse{TotalStake: channels.StakeAmount}, nil
}

// ReduceStakeFromChannels initiates a stake reduction with an unbonding period.
// The reduced amount is locked until reduceUnlockHeight, then claimable.
func (k Keeper) ReduceStakeFromChannels(ctx sdk.Context, sender string, amount sdk.Coin) error {
	channels, found := k.GetChannels(ctx, sender)
	if !found {
		return types.ErrChannelsNotFound
	}
	if channels.Status == types.ChannelsStatus_CHANNELS_STATUS_SUSPENDED {
		return types.ErrChannelsSuspended
	}
	if k.HasDeregisterRequest(ctx, sender) {
	...
	minStakeInt := sdkmath.NewIntFromBigInt(new(big.Int).Mul(big.NewInt(int64(params.MinRegisterStake)), oneAxon))
	minStake := sdk.NewCoin("aaxon", minStakeInt)
	remaining := channels.StakeAmount.Sub(amount)
	...
	channels.StakeAmount = remaining
	k.SetChannels(ctx, channels)

	k.setPendingReduce(ctx, sender, amount.Amount, unlockHeight)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"channels_stake_reduce_initiated",
		sdk.NewAttribute("address", sender),
...
// GetStakeInfo returns stake details for an channels.
func (k Keeper) GetStakeInfo(ctx sdk.Context, address string) (totalStake sdkmath.Int, pendingReduce sdkmath.Int, reduceUnlockHeight int64, found bool) {
	channels, channelsFound := k.GetChannels(ctx, address)
	if !channelsFound {
		return sdkmath.ZeroInt(), sdkmath.ZeroInt(), 0, false
	}
	totalStake = channels.StakeAmount.Amount
...
func (k Keeper) UpdateReputation(ctx sdk.Context, address string, delta int64) {
	...
	params := k.GetParams(ctx)
	newRep := int64(channels.Reputation) + delta
	if newRep < 0 {
		newRep = 0
	}
	...
	channels.Reputation = uint64(newRep)
	k.SetChannels(ctx, channels)
}
...
const walletKVPrefix = "ChannelsWallet/"
