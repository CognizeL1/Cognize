...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.Channelss) != 0 {
		t.Errorf("expected 0 channelss in default genesis, got %d", len(gs.Channelss))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	channelsKey := types.KeyChannels(addr)
	if string(channelsKey) != types.ChannelsKeyPrefix+addr {
		t.Errorf("KeyChannels mismatch: %s", channelsKey)
	}
