...
// ProcessL1Reputation is called at epoch end. It evaluates all channelss and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		addr := channels.Address
		if k.shouldFreezeChannelsReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old channels.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		if k.shouldFreezeChannelsReputationDuringDeregister(ctx, channels.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, channels.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if channels.Reputation != legacy {
			channels.Reputation = legacy
...
// dual-layer L1/L2 score stores when an channels is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when channels goes offline.
...
// isEpochCheater checks if an channels was flagged as a cheater in the given epoch.
