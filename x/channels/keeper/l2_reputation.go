...
const (
	...
	DefaultL2BudgetPerChannelsMillis int64 = 100     // 0.1 per channels
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporterChannels, found := k.GetChannels(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporterChannels.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetChannels, found := k.GetChannels(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetChannels.Status == types.ChannelsStatus_CHANNELS_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	channelsCount := int64(0)
	k.IterateChannelss(ctx, func(_ types.Channels) bool {
		channelsCount++
		return false
	})

	budgetPerChannels := l2BudgetPerChannelsMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPerChannels * channelsCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("channelss_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPerChannelsMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPerChannels, types.DefaultParams().L2BudgetPerChannels, DefaultL2BudgetPerChannelsMillis)
}
