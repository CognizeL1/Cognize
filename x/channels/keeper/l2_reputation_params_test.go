...
func setTestChannels(k Keeper, ctx sdk.Context, address string, registeredAt int64) {
	k.SetChannels(ctx, types.Channels{
		Address:          address,
		ChannelsId:          address,
...
func TestSubmitL2ReportUsesConfiguredThresholdsAndWeights(t *testing.T) {
	...
	setTestChannels(k, ctx, "reporter", ctx.BlockHeight())
...
func TestSubmitL2ReportRejectsTooNewUsingConfiguredAccountAge(t *testing.T) {
	...
	setTestChannels(k, ctx, "reporter", 100)
	setTestChannels(k, ctx, "target", 1)
	k.SetL1Score(ctx, "reporter", 10_000)
...
func TestBootstrapLegacyReputationSeedsDualLayerScores(t *testing.T) {
	...
	k.BootstrapLegacyReputation(ctx, "channels", 50)

	if got := k.GetL1Score(ctx, "channels"); got != 40_000 {
		t.Fatalf("expected l1 score 40000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "channels"); got != 10_000 {
		t.Fatalf("expected l2 score 10000, got %d", got)
	}
	if got := k.GetTotalReputation(ctx, "channels"); got != 50_000 {
...
func TestApplyReputationDecayRespectsZeroParams(t *testing.T) {
	...
	k.SetL1Score(ctx, "channels", 10_000)
	k.SetL2Score(ctx, "channels", 5_000)

	k.ApplyReputationDecay(ctx)

	if got := k.GetL1Score(ctx, "channels"); got != 10_000 {
		t.Fatalf("expected l1 score to remain 10000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "channels"); got != 5_000 {
