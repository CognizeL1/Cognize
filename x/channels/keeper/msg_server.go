...
func (k msgServer) AddStake(goCtx context.Context, msg *types.MsgAddStake) (*types.MsgAddStakeResponse, error) {
	...
	return k.Keeper.AddStakeToChannels(ctx, msg.Sender, msg.Stake, senderAddr)
}

func (k msgServer) UpdateChannels(goCtx context.Context, msg *types.MsgUpdateChannels) (*types.MsgUpdateChannelsResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	channels, found := k.GetChannels(ctx, msg.Sender)
	if !found {
	...
	if msg.Capabilities != "" {
		...
		channels.Capabilities = caps
	}
	if msg.Model != "" {
		channels.Model = msg.Model
	}
...
func (k msgServer) Heartbeat(goCtx context.Context, msg *types.MsgHeartbeat) (*types.MsgHeartbeatResponse, error) {
	...
	if ctx.BlockHeight()-channels.LastHeartbeat < params.HeartbeatInterval {
		return nil, types.ErrHeartbeatTooFrequent
	}

	channels.LastHeartbeat = ctx.BlockHeight()
	channels.Status = types.ChannelsStatus_CHANNELS_STATUS_ONLINE
	k.SetChannels(ctx, channels)
...
func (k msgServer) ReduceStake(goCtx context.Context, msg *types.MsgReduceStake) (*types.MsgReduceStakeResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := k.Keeper.ReduceStakeFromChannels(ctx, msg.Sender, msg.Amount); err != nil {
		return nil, err
	}
...
func (k msgServer) RevealAIChallengeResponse(goCtx context.Context, msg *types.MsgRevealAIChallengeResponse) (*types.MsgRevealAIChallengeResponseResponse, error) {
	...
	if _, found := k.GetChannels(ctx, msg.Sender); !found {
		return nil, types.ErrChannelsNotFound
	}
	if k.HasDeregisterRequest(ctx, msg.Sender) {
...
func generateChannelsID(address string, blockHeight int64) string {
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s:%d", address, blockHeight)))
	return fmt.Sprintf("channels-%s", hex.EncodeToString(hash[:8]))
}
