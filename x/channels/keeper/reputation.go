...
// IncrementEpochActivity increments the transaction count for an channels in the current epoch.
...
// GetEpochActivity returns the transaction count for an channels in a given epoch.
...
// ProcessEpochReputation updates reputation for all channelss at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		if channels.Status == types.ChannelsStatus_CHANNELS_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: channels.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updatedChannels, found := k.GetChannels(ctx, u.address)
		if found && updatedChannels.Reputation == 0 {
			k.handleZeroReputation(ctx, updatedChannels, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the channels.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, channels types.Channels, params types.Params) {
	var burnedAtRegister sdk.Coin
	if channels.BurnedAtRegister.Denom != "" && channels.BurnedAtRegister.IsPositive() {
		burnedAtRegister = channels.BurnedAtRegister
	} else {
		// Fallback for legacy channelss without snapshot
	...
	if channels.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = channels.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", channels.Address, "error", err)
		} else {
			burned = true
		}
	}

	channels.Status = types.ChannelsStatus_CHANNELS_STATUS_SUSPENDED
	if burned || !remaining.IsPositive() {
		channels.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.SetChannels(ctx, channels)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"channels_stake_burned",
		sdk.NewAttribute("address", channels.Address),
