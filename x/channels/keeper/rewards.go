...
// DistributeEpochRewards distributes contribution rewards to online channelss
...
func (k Keeper) DistributeEpochRewards(ctx sdk.Context, epoch uint64) {
	...
	type weightedChannels struct {
		address string
		weight  *big.Int
	}

	var channelss []weightedChannels
	...
	k.IterateChannelss(ctx, func(channels types.Channels) bool {
		...
		stakeAmount := channels.StakeAmount.Amount.BigInt()

		repBonus := reputationBonusPercent(channels.Reputation)
		aiBonus := k.GetAIBonus(ctx, channels.Address)
		...
		channelss = append(channelss, weightedChannels{
	...
	if totalWeight.Sign() <= 0 || len(channelss) == 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("channelss_count", fmt.Sprintf("%d", len(channelss))),
	))
}
