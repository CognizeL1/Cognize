...
func TestSubmitL2ReportUsesFullWeightOnlyForIndexedEvidence(t *testing.T) {
	...
	setTestChannels(k, ctx, "reporter", 1)
	setTestChannels(k, ctx, "target", 1)
...
func TestSubmitL2ReportFallsBackWhenEvidenceMissingOrInvalid(t *testing.T) {
	...
	setTestChannels(k, ctx, "reporter1", 1)
	setTestChannels(k, ctx, "target1", 1)
	...
	setTestChannels(k, ctx, "reporter2", 1)
	setTestChannels(k, ctx, "target2", 1)
...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "channels1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "channels1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "channels1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "channels1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "channels1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "channels1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "channels1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "channels1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "channels1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "channels1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("channels1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("channels1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("channels1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("channels1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("channels1", 2)) || !store.Has(dailyRegisterKey("channels1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("channels1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("channels1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 channelss × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "channels" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("channels0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, channels.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, channels.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, channels.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, channels)
	if score != 801105 {
...
func TestEvaluateEpochChallengesScoresCorrectlyBeforeUpgrade(t *testing.T) {
	...
	// Register the validator as channels
	setTestChannels(k, ctx, wrongResp.ValidatorAddress, 1)
...
func TestHistoricalReplayConsistency(t *testing.T) {
	...
	setTestChannels(k, ctx, resp1.ValidatorAddress, 1)
	...
	setTestChannels(k, ctx, resp2.ValidatorAddress, 1)
