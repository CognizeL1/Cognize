...
func TestKeyChannels(t *testing.T) {
	key := types.KeyChannels("axon1test")
	want := types.ChannelsKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("KeyChannels = %q, want %q", key, want)
	}
}

func TestKeyChannelsDifferentAddresses(t *testing.T) {
	k1 := types.KeyChannels("axon1aaa")
	k2 := types.KeyChannels("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"ChannelsKeyPrefix", types.ChannelsKeyPrefix},
...
func TestDefaultGenesisEmptyChannelss(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.Channelss) != 0 {
		t.Errorf("default genesis should have 0 channelss, got %d", len(gs.Channelss))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "channels" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "channels")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErrChannelsAlreadyRegistered", types.ErrChannelsAlreadyRegistered},
		{"ErrChannelsNotFound", types.ErrChannelsNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErrChannelsSuspended", types.ErrChannelsSuspended},
