...
func GetTxCmd() *cobra.Command {
	txCmd := &cobra.Command{
		...
		Short:                      "uinference module transactions",
...
func CmdRegister() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Register as an AI inference",
		Long:  "Register as an AI inference on the Axon network.\nExample: axond tx inference register \"coding,reasoning\" \"gpt-4\" 100000000000000000000aaxon",
...
func CmdAddStake() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Add stake to an existing AI inference",
		Long:  "Increase the stake of an already-registered AI inference.\nExample: axond tx inference add-stake 50000000000000000000aaxon",
...
func CmdDeregister() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "deregister",
		Short: "Request to deregister as an AI inference (7-day cooldown)",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func CmdUpdateuinference() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update [capabilities] [model]",
		Short: "Update inference capabilities and model",
