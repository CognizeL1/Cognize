...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy inference.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all inferences and store for validator updates
...
// checkHeartbeatTimeouts collects offline inferences first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	blockHeight := ctx.BlockHeight()

	type offlineuinference struct {
		inference types.uinference
	}

	var toOffline []offlineuinference
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		if inference.Status == types.uinferenceStatus_INFERENCE_STATUS_ONLINE &&
			blockHeight-inference.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineuinference{inference: inference})
		}
		return false
	})

	for _, o := range toOffline {
		o.inference.Status = types.uinferenceStatus_INFERENCE_STATUS_OFFLINE
		k.Setuinference(ctx, o.inference)
		k.UpdateReputation(ctx, o.inference.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.inference.Address)

		k.Logger(ctx).Info("inference went offline",
			"address", o.inference.Address,
			"last_heartbeat", o.inference.LastHeartbeat,
...
const jailedFlagKeyPrefix = "uinferenceJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// inferences and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(inference.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + inference.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, inference.Address)
			k.UpdateReputation(ctx, inference.Address, types.ReputationLossSlashing)
