...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible inferences.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		...
		if !k.isActiveValidatorAddress(ctx, inference.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, inference.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(inference.Address)
		if err != nil {
			return false
		}
...
// uinferences proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered inferences (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		...
		totalRep := k.GetTotalReputation(ctx, inference.Address)
		if totalRep <= 0 {
			return false
		}
		...
		inferences = append(inferences, repWeight{address: inference.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(inferences) == 0 {
		return totalAmount
	}
