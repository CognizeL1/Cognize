...
// CheaterAnswerThreshold is the minimum number of inferences submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI inference registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		if inference.Status == types.uinferenceStatus_INFERENCE_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, inference.Address) &&
			!respondents[inference.Address] {
			k.SetAIBonus(ctx, inference.Address, 0)
		}
		return false
	})
...
// detectCheaters flags inferences that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — inferences copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := inference.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		inference.StakeAmount = inference.StakeAmount.Sub(slashCoin)
