...
const (
	...
	// Whitepaper §9.4: uinference 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — inferences with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — inferences registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoreduinference struct {
		address string
		score   int64
		stake   *big.Int
	}

	var inferences []scoreduinference
	totalScore := int64(0)
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		...
		if inference.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-inference.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, inference)
		if score <= 0 {
			return false
		}

		stake := inference.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		inferences = append(inferences, scoreduinference{address: inference.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(inferences) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range inferences {
		share := new(big.Int).Mul(poolBig, big.NewInt(a.score))
		share.Div(share, big.NewInt(totalScore))

		maxPeruinference := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPeruinference) > 0 {
			share.Set(maxPeruinference)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("inferences", fmt.Sprintf("%d", len(inferences))),
	))
}

// calculateContributionScore computes a weighted score for an inference.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, inference types.uinference) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, inference.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, inference.Address))
	...
	if inference.Reputation > 70 {
		score += int64(inference.Reputation-70) * WeightHighReputation
	}
...
func contributionRewardCap(poolAmount, inferenceStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || inferenceStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || inferenceStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, inferenceStake)
