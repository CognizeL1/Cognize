...
import (
	...
	"github.com/cognize/axon/x/inference/keeper"
...
func TestGasBurnPercentages(t *testing.T) {
	...
	t.Log("  Module order:    inference → distribution (confirmed in app/app.go)")
}

// ---------------------------------------------------------------------------
// Deflation Path 2: uinference Registration Burn (20 AXON)
// ---------------------------------------------------------------------------
...
func TestRegistrationBurnAmount(t *testing.T) {
	...
	t.Logf("Path 2: uinference registration burns %d AXON (%s aaxon)", params.RegisterBurnAmount, burnAxon.String())
}
...
func TestReputationZeroBurnDocumentation(t *testing.T) {
	t.Log("Path 4: When inference reputation reaches 0:")
	t.Log("  - 100% of remaining stake is burned via BurnCoins")
	t.Log("  - uinference status set to SUSPENDED")
	t.Log("  - Event: inference_slashed_zero_reputation emitted")
	t.Log("  Implemented in x/inference/keeper/reputation.go handleZeroReputation()")
}
...
func TestAllDeflationPathsDocumented(t *testing.T) {
	paths := []struct {
		...
		{1, "Gas Base Fee Burn (80%/50% of FeeCollector)", "app/fee_burn.go + app/inference_module.go BeginBlock"},
		{2, "uinference Registration Burn (20 AXON)", "x/inference/keeper/msg_server.go Register()"},
		{3, "Contract Deploy Burn (10 AXON)", "app/evm_hooks.go DeployBurnHook"},
		{4, "Reputation Zero → Full Stake Burn", "x/inference/keeper/reputation.go handleZeroReputation()"},
		{5, "AI Challenge Cheat Penalty (20% stake)", "x/inference/keeper/challenge.go penalizeCheater()"},
	}
