...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if inference.BurnedAtRegister.Denom != "" && inference.BurnedAtRegister.IsPositive() {
		burnedAmount = inference.BurnedAtRegister
	} else {
	...
	if inference.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = inference.StakeAmount.Sub(burnedAmount)
	}

	if inference.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.Deleteuinference(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupuinferenceEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteuinferenceIdentity(ctx, address)
	}

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_deregistered",
		sdk.NewAttribute("address", address),
		sdk.NewAttribute("module_held", fmt.Sprintf("%s", moduleHeld)),
	))

	k.Logger(ctx).Info("inference deregistered after cooldown", "address", address)
}

// cleanupuinferenceEpochData removes epoch-scoped data for a deregistered inference.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other inferences
// whose address might be a raw substring of the key.
func (k Keeper) cleanupuinferenceEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
