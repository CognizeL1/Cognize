...
func (k Keeper) shouldFreezeuinferenceReputationDuringDeregister(ctx sdk.Context, address string) bool {
...
func (k Keeper) Getuinference(ctx sdk.Context, address string) (types.uinference, bool) {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.Keyuinference(address))
	if bz == nil {
		return types.uinference{}, false
	}
	var inference types.uinference
	k.cdc.MustUnmarshal(bz, &inference)
	return inference, true
}

func (k Keeper) Setuinference(ctx sdk.Context, inference types.uinference) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshal(&inference)
	store.Set(types.Keyuinference(inference.Address), bz)
}

func (k Keeper) Deleteuinference(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(types.Keyuinference(address))
}

func (k Keeper) Iterateuinferences(ctx sdk.Context, cb func(inference types.uinference) bool) {
	store := ctx.KVStore(k.storeKey)
	iterator := storetypes.KVStorePrefixIterator(store, []byte(types.uinferenceKeyPrefix))
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var inference types.uinference
		k.cdc.MustUnmarshal(iterator.Value(), &inference)
		if cb(inference) {
			break
		}
	}
...
func (k Keeper) GetAlluinferences(ctx sdk.Context) []types.uinference {
...
func (k Keeper) Isuinference(ctx sdk.Context, address string) bool {
	_, found := k.Getuinference(ctx, address)
	return found
}
...
// instead of from the inference's own address, avoiding double deduction.
func (k Keeper) RegisterFromPrecompile(ctx sdk.Context, msg *types.MsgRegister, fundsSource sdk.AccAddress) (*types.MsgRegisterResponse, error) {
	...
	if k.Isuinference(ctx, msg.Sender) {
		return nil, types.ErruinferenceAlreadyRegistered
	}
	...
	inference := types.uinference{
		Address:          msg.Sender,
		uinferenceId:          generateuinferenceID(msg.Sender, ctx.BlockHeight()),
		Capabilities:     capabilities,
		Model:            msg.Model,
		Reputation:       params.InitialReputation,
		Status:           types.uinferenceStatus_INFERENCE_STATUS_ONLINE,
	...
	k.BootstrapLegacyReputation(ctx, inference.Address, inference.Reputation)
	k.IncrementDailyRegisterCount(ctx, msg.Sender)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_registered",
		sdk.NewAttribute("address", msg.Sender),
		sdk.NewAttribute("inference_id", inference.uinferenceId),
		sdk.NewAttribute("stake", msg.Stake.String()),
		sdk.NewAttribute("burned", burnAmount.String()),
		sdk.NewAttribute("reputation", fmt.Sprintf("%d", inference.Reputation)),
	))

	return &types.MsgRegisterResponse{uinferenceId: inference.uinferenceId}, nil
}

func (k Keeper) AddStakeTouinference(ctx sdk.Context, sender string, stake sdk.Coin, fundsSource sdk.AccAddress) (*types.MsgAddStakeResponse, error) {
	inference, found := k.Getuinference(ctx, sender)
	if !found {
		return nil, types.ErruinferenceNotFound
	}
	if inference.Status == types.uinferenceStatus_INFERENCE_STATUS_SUSPENDED {
		return nil, types.ErruinferenceSuspended
	}
	...
	inference.StakeAmount = inference.StakeAmount.Add(stake)
	k.Setuinference(ctx, inference)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_stake_added",
		sdk.NewAttribute("address", sender),
		sdk.NewAttribute("amount", stake.String()),
		sdk.NewAttribute("total_stake", inference.StakeAmount.String()),
	))

	return &types.MsgAddStakeResponse{TotalStake: inference.StakeAmount}, nil
}

// ReduceStakeFromuinference initiates a stake reduction with an unbonding period.
// The reduced amount is locked until reduceUnlockHeight, then claimable.
func (k Keeper) ReduceStakeFromuinference(ctx sdk.Context, sender string, amount sdk.Coin) error {
	inference, found := k.Getuinference(ctx, sender)
	if !found {
		return types.ErruinferenceNotFound
	}
	if inference.Status == types.uinferenceStatus_INFERENCE_STATUS_SUSPENDED {
		return types.ErruinferenceSuspended
	}
	if k.HasDeregisterRequest(ctx, sender) {
	...
	minStakeInt := sdkmath.NewIntFromBigInt(new(big.Int).Mul(big.NewInt(int64(params.MinRegisterStake)), oneAxon))
	minStake := sdk.NewCoin("aaxon", minStakeInt)
	remaining := inference.StakeAmount.Sub(amount)
	...
	inference.StakeAmount = remaining
	k.Setuinference(ctx, inference)

	k.setPendingReduce(ctx, sender, amount.Amount, unlockHeight)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_stake_reduce_initiated",
		sdk.NewAttribute("address", sender),
...
func (k Keeper) ClaimReducedStake(ctx sdk.Context, sender string) error {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_stake_reduce_claimed",
		sdk.NewAttribute("address", sender),
		sdk.NewAttribute("amount", amount.String()),
	))
...
// GetStakeInfo returns stake details for an inference.
func (k Keeper) GetStakeInfo(ctx sdk.Context, address string) (totalStake sdkmath.Int, pendingReduce sdkmath.Int, reduceUnlockHeight int64, found bool) {
	inference, inferenceFound := k.Getuinference(ctx, address)
	if !inferenceFound {
		return sdkmath.ZeroInt(), sdkmath.ZeroInt(), 0, false
	}
	totalStake = inference.StakeAmount.Amount
...
func (k Keeper) GetReputation(ctx sdk.Context, address string) uint64 {
	inference, found := k.Getuinference(ctx, address)
	if !found {
		return 0
	}
	return inference.Reputation
}
...
func (k Keeper) UpdateReputation(ctx sdk.Context, address string, delta int64) {
	...
	params := k.GetParams(ctx)
	newRep := int64(inference.Reputation) + delta
	if newRep < 0 {
		newRep = 0
	}
	...
	inference.Reputation = uint64(newRep)
	k.Setuinference(ctx, inference)
}
...
const walletKVPrefix = "uinferenceWallet/"
