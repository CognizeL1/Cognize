...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.uinferences) != 0 {
		t.Errorf("expected 0 inferences in default genesis, got %d", len(gs.uinferences))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	inferenceKey := types.Keyuinference(addr)
	if string(inferenceKey) != types.uinferenceKeyPrefix+addr {
		t.Errorf("Keyuinference mismatch: %s", inferenceKey)
	}
