...
// ProcessL1Reputation is called at epoch end. It evaluates all inferences and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		addr := inference.Address
		if k.shouldFreezeuinferenceReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old inference.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		if k.shouldFreezeuinferenceReputationDuringDeregister(ctx, inference.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, inference.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if inference.Reputation != legacy {
			inference.Reputation = legacy
...
// dual-layer L1/L2 score stores when an inference is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when inference goes offline.
...
// isEpochCheater checks if an inference was flagged as a cheater in the given epoch.
