...
const (
	...
	DefaultL2BudgetPeruinferenceMillis int64 = 100     // 0.1 per inference
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporteruinference, found := k.Getuinference(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporteruinference.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetuinference, found := k.Getuinference(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetuinference.Status == types.uinferenceStatus_INFERENCE_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	inferenceCount := int64(0)
	k.Iterateuinferences(ctx, func(_ types.uinference) bool {
		inferenceCount++
		return false
	})

	budgetPeruinference := l2BudgetPeruinferenceMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPeruinference * inferenceCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("inferences_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPeruinferenceMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPeruinference, types.DefaultParams().L2BudgetPeruinference, DefaultL2BudgetPeruinferenceMillis)
}
