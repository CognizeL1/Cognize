...
// ComputeAllMiningPowers iterates over all inferences and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		stake := inference.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, inference.Address)
