...
func (k msgServer) AddStake(goCtx context.Context, msg *types.MsgAddStake) (*types.MsgAddStakeResponse, error) {
	...
	return k.Keeper.AddStakeTouinference(ctx, msg.Sender, msg.Stake, senderAddr)
}

func (k msgServer) Updateuinference(goCtx context.Context, msg *types.MsgUpdateuinference) (*types.MsgUpdateuinferenceResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	inference, found := k.Getuinference(ctx, msg.Sender)
	if !found {
	...
	k.Setuinference(ctx, inference)
	return &types.MsgUpdateuinferenceResponse{}, nil
}
...
func (k msgServer) Heartbeat(goCtx context.Context, msg *types.MsgHeartbeat) (*types.MsgHeartbeatResponse, error) {
	...
	if ctx.BlockHeight()-inference.LastHeartbeat < params.HeartbeatInterval {
		return nil, types.ErrHeartbeatTooFrequent
	}

	inference.LastHeartbeat = ctx.BlockHeight()
	inference.Status = types.uinferenceStatus_INFERENCE_STATUS_ONLINE
	k.Setuinference(ctx, inference)
...
func (k msgServer) Deregister(goCtx context.Context, msg *types.MsgDeregister) (*types.MsgDeregisterResponse, error) {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"inference_deregister_requested",
		sdk.NewAttribute("address", msg.Sender),
...
func (k msgServer) ReduceStake(goCtx context.Context, msg *types.MsgReduceStake) (*types.MsgReduceStakeResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := k.Keeper.ReduceStakeFromuinference(ctx, msg.Sender, msg.Amount); err != nil {
		return nil, err
	}
...
func (k msgServer) RevealAIChallengeResponse(goCtx context.Context, msg *types.MsgRevealAIChallengeResponse) (*types.MsgRevealAIChallengeResponseResponse, error) {
	...
	if _, found := k.Getuinference(ctx, msg.Sender); !found {
		return nil, types.ErruinferenceNotFound
	}
	if k.HasDeregisterRequest(ctx, msg.Sender) {
...
func generateuinferenceID(address string, blockHeight int64) string {
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s:%d", address, blockHeight)))
	return fmt.Sprintf("inference-%s", hex.EncodeToString(hash[:8]))
}
