...
// DistributeEpochRewards distributes contribution rewards to online inferences
...
func (k Keeper) DistributeEpochRewards(ctx sdk.Context, epoch uint64) {
	...
	type weighteduinference struct {
		address string
		weight  *big.Int
	}

	var inferences []weighteduinference
	...
	k.Iterateuinferences(ctx, func(inference types.uinference) bool {
		...
		stakeAmount := inference.StakeAmount.Amount.BigInt()

		repBonus := reputationBonusPercent(inference.Reputation)
		aiBonus := k.GetAIBonus(ctx, inference.Address)
		...
		inferences = append(inferences, weighteduinference{
	...
	if totalWeight.Sign() <= 0 || len(inferences) == 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("inferences_count", fmt.Sprintf("%d", len(inferences))),
	))
}
