...
func TestSubmitL2ReportUsesFullWeightOnlyForIndexedEvidence(t *testing.T) {
	...
	setTestuinference(k, ctx, "reporter", 1)
	setTestuinference(k, ctx, "target", 1)
...
func TestSubmitL2ReportFallsBackWhenEvidenceMissingOrInvalid(t *testing.T) {
	...
	setTestuinference(k, ctx, "reporter1", 1)
	setTestuinference(k, ctx, "target1", 1)
	...
	setTestuinference(k, ctx, "reporter2", 1)
	setTestuinference(k, ctx, "target2", 1)
...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "inference1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "inference1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "inference1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "inference1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "inference1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "inference1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "inference1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "inference1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "inference1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "inference1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("inference1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("inference1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("inference1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("inference1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("inference1", 2)) || !store.Has(dailyRegisterKey("inference1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("inference1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("inference1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 inferences × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "inference" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("inference0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, inference.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, inference.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, inference.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, inference)
	if score != 801105 {
...
func TestEvaluateEpochChallengesScoresCorrectlyBeforeUpgrade(t *testing.T) {
	...
	// Register the validator as inference
	setTestuinference(k, ctx, wrongResp.ValidatorAddress, 1)
...
func TestHistoricalReplayConsistency(t *testing.T) {
	...
	setTestuinference(k, ctx, resp1.ValidatorAddress, 1)
	...
	setTestuinference(k, ctx, resp2.ValidatorAddress, 1)
