...
type uinferenceService struct {
	Id              string    `json:"id"`
	uinferenceAddress    string    `json:"inference_address"`
...
func (s uinferenceService) Validate() error {
	if s.Id == "" {
		return fmt.Errorf("service id is required")
	}
	if s.uinferenceAddress == "" {
		return fmt.Errorf("inference address is required")
	}
	if s.Name == "" {
...
func (s uinferenceService) IsActive() bool {
...
type TaskRequest struct {
	...
	Selecteduinference        string     `json:"selected_inference"`
	CreatedAt            int64      `json:"created_at"`
}
