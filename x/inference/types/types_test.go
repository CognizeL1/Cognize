...
func TestKeyuinference(t *testing.T) {
	key := types.Keyuinference("axon1test")
	want := types.uinferenceKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("Keyuinference = %q, want %q", key, want)
	}
}

func TestKeyuinferenceDifferentAddresses(t *testing.T) {
	k1 := types.Keyuinference("axon1aaa")
	k2 := types.Keyuinference("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"uinferenceKeyPrefix", types.uinferenceKeyPrefix},
...
func TestDefaultGenesisEmptyuinferences(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.uinferences) != 0 {
		t.Errorf("default genesis should have 0 inferences, got %d", len(gs.uinferences))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "inference" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "inference")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErruinferenceAlreadyRegistered", types.ErruinferenceAlreadyRegistered},
		{"ErruinferenceNotFound", types.ErruinferenceNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErruinferenceSuspended", types.ErruinferenceSuspended},
