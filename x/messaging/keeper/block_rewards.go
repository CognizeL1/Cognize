...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible messagings.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		...
		if !k.isActiveValidatorAddress(ctx, messaging.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, messaging.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(messaging.Address)
		if err != nil {
			return false
		}
...
// Messagings proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered messagings (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		...
		totalRep := k.GetTotalReputation(ctx, messaging.Address)
		if totalRep <= 0 {
			return false
		}
		...
		messagings = append(messagings, repWeight{address: messaging.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(messagings) == 0 {
		return totalAmount
	}
