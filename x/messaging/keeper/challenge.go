...
// CheaterAnswerThreshold is the minimum number of messagings submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI messaging registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		if messaging.Status == types.MessagingStatus_MESSAGING_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, messaging.Address) &&
			!respondents[messaging.Address] {
			k.SetAIBonus(ctx, messaging.Address, 0)
		}
		return false
	})
...
// detectCheaters flags messagings that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — messagings copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := messaging.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		messaging.StakeAmount = messaging.StakeAmount.Sub(slashCoin)
