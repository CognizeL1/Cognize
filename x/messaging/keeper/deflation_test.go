...
import (
	...
	"github.com/cognize/axon/x/messaging/keeper"
...
func TestGasBurnPercentages(t *testing.T) {
	...
	t.Log("  Module order:    messaging → distribution (confirmed in app/app.go)")
}

// ---------------------------------------------------------------------------
// Deflation Path 2: Messaging Registration Burn (20 AXON)
// ---------------------------------------------------------------------------
...
func TestRegistrationBurnAmount(t *testing.T) {
	...
	t.Logf("Path 2: Messaging registration burns %d AXON (%s aaxon)", params.RegisterBurnAmount, burnAxon.String())
}
...
func TestReputationZeroBurnDocumentation(t *testing.T) {
	t.Log("Path 4: When messaging reputation reaches 0:")
	t.Log("  - 100% of remaining stake is burned via BurnCoins")
	t.Log("  - Messaging status set to SUSPENDED")
	t.Log("  - Event: messaging_slashed_zero_reputation emitted")
	t.Log("  Implemented in x/messaging/keeper/reputation.go handleZeroReputation()")
}
...
func TestAllDeflationPathsDocumented(t *testing.T) {
	paths := []struct {
		...
		{1, "Gas Base Fee Burn (80%/50% of FeeCollector)", "app/fee_burn.go + app/messaging_module.go BeginBlock"},
		{2, "Messaging Registration Burn (20 AXON)", "x/messaging/keeper/msg_server.go Register()"},
		{3, "Contract Deploy Burn (10 AXON)", "app/evm_hooks.go DeployBurnHook"},
		{4, "Reputation Zero → Full Stake Burn", "x/messaging/keeper/reputation.go handleZeroReputation()"},
		{5, "AI Challenge Cheat Penalty (20% stake)", "x/messaging/keeper/challenge.go penalizeCheater()"},
	}
