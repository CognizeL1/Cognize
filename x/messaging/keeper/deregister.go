...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if messaging.BurnedAtRegister.Denom != "" && messaging.BurnedAtRegister.IsPositive() {
		burnedAmount = messaging.BurnedAtRegister
	} else {
	...
	if messaging.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = messaging.StakeAmount.Sub(burnedAmount)
	}

	if messaging.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.DeleteMessaging(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupMessagingEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteMessagingIdentity(ctx, address)
	}

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_deregistered",
		sdk.NewAttribute("address", address),
		sdk.NewAttribute("module_held", fmt.Sprintf("%s", moduleHeld)),
	))

	k.Logger(ctx).Info("messaging deregistered after cooldown", "address", address)
}

// cleanupMessagingEpochData removes epoch-scoped data for a deregistered messaging.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other messagings
// whose address might be a raw substring of the key.
func (k Keeper) cleanupMessagingEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
