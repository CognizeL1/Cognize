...
func (k Keeper) shouldFreezeMessagingReputationDuringDeregister(ctx sdk.Context, address string) bool {
...
func (k Keeper) GetMessaging(ctx sdk.Context, address string) (types.Messaging, bool) {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.KeyMessaging(address))
	if bz == nil {
		return types.Messaging{}, false
	}
	var messaging types.Messaging
	k.cdc.MustUnmarshal(bz, &messaging)
	return messaging, true
}

func (k Keeper) SetMessaging(ctx sdk.Context, messaging types.Messaging) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshal(&messaging)
	store.Set(types.KeyMessaging(messaging.Address), bz)
}

func (k Keeper) DeleteMessaging(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(types.KeyMessaging(address))
}

func (k Keeper) IterateMessagings(ctx sdk.Context, cb func(messaging types.Messaging) bool) {
	store := ctx.KVStore(k.storeKey)
	iterator := storetypes.KVStorePrefixIterator(store, []byte(types.MessagingKeyPrefix))
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var messaging types.Messaging
		k.cdc.MustUnmarshal(iterator.Value(), &messaging)
		if cb(messaging) {
			break
		}
	}
...
func (k Keeper) GetAllMessagings(ctx sdk.Context) []types.Messaging {
...
func (k Keeper) IsMessaging(ctx sdk.Context, address string) bool {
	_, found := k.GetMessaging(ctx, address)
	return found
}
...
// instead of from the messaging's own address, avoiding double deduction.
func (k Keeper) RegisterFromPrecompile(ctx sdk.Context, msg *types.MsgRegister, fundsSource sdk.AccAddress) (*types.MsgRegisterResponse, error) {
	...
	if k.IsMessaging(ctx, msg.Sender) {
		return nil, types.ErrMessagingAlreadyRegistered
	}
	...
	messaging := types.Messaging{
		Address:          msg.Sender,
		MessagingId:          generateMessagingID(msg.Sender, ctx.BlockHeight()),
	...
	k.SetMessaging(ctx, messaging)
	k.BootstrapLegacyReputation(ctx, messaging.Address, messaging.Reputation)
	k.IncrementDailyRegisterCount(ctx, msg.Sender)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_registered",
		sdk.NewAttribute("address", msg.Sender),
		sdk.NewAttribute("messaging_id", messaging.MessagingId),
		sdk.NewAttribute("stake", msg.Stake.String()),
		sdk.NewAttribute("burned", burnAmount.String()),
		sdk.NewAttribute("reputation", fmt.Sprintf("%d", messaging.Reputation)),
	))

	return &types.MsgRegisterResponse{MessagingId: messaging.MessagingId}, nil
}

func (k Keeper) AddStakeToMessaging(ctx sdk.Context, sender string, stake sdk.Coin, fundsSource sdk.AccAddress) (*types.MsgAddStakeResponse, error) {
	messaging, found := k.GetMessaging(ctx, sender)
	if !found {
		return nil, types.ErrMessagingNotFound
	}
	if messaging.Status == types.MessagingStatus_MESSAGING_STATUS_SUSPENDED {
		return nil, types.ErrMessagingSuspended
	}
	...
	messaging.StakeAmount = messaging.StakeAmount.Add(stake)
	k.SetMessaging(ctx, messaging)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_stake_added",
		sdk.NewAttribute("address", sender),
		sdk.NewAttribute("amount", stake.String()),
		sdk.NewAttribute("total_stake", messaging.StakeAmount.String()),
	))

	return &types.MsgAddStakeResponse{TotalStake: messaging.StakeAmount}, nil
}

// ReduceStakeFromMessaging initiates a stake reduction with an unbonding period.
// The reduced amount is locked until reduceUnlockHeight, then claimable.
func (k Keeper) ReduceStakeFromMessaging(ctx sdk.Context, sender string, amount sdk.Coin) error {
	messaging, found := k.GetMessaging(ctx, sender)
	if !found {
		return types.ErrMessagingNotFound
	}
	if messaging.Status == types.MessagingStatus_MESSAGING_STATUS_SUSPENDED {
		return types.ErrMessagingSuspended
	}
	if k.HasDeregisterRequest(ctx, sender) {
	...
	minStakeInt := sdkmath.NewIntFromBigInt(new(big.Int).Mul(big.NewInt(int64(params.MinRegisterStake)), oneAxon))
	minStake := sdk.NewCoin("aaxon", minStakeInt)
	remaining := messaging.StakeAmount.Sub(amount)
	...
	messaging.StakeAmount = remaining
	k.SetMessaging(ctx, messaging)

	k.setPendingReduce(ctx, sender, amount.Amount, unlockHeight)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_stake_reduce_initiated",
		sdk.NewAttribute("address", sender),
...
func (k Keeper) ClaimReducedStake(ctx sdk.Context, sender string) error {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_stake_reduce_claimed",
		sdk.NewAttribute("address", sender),
		sdk.NewAttribute("amount", amount.String()),
	))
...
// GetStakeInfo returns stake details for an messaging.
func (k Keeper) GetStakeInfo(ctx sdk.Context, address string) (totalStake sdkmath.Int, pendingReduce sdkmath.Int, reduceUnlockHeight int64, found bool) {
	messaging, messagingFound := k.GetMessaging(ctx, address)
	if !messagingFound {
		return sdkmath.ZeroInt(), sdkmath.ZeroInt(), 0, false
	}
	totalStake = messaging.StakeAmount.Amount
...
func (k Keeper) GetReputation(ctx sdk.Context, address string) uint64 {
	messaging, found := k.GetMessaging(ctx, address)
	if !found {
		return 0
	}
	return messaging.Reputation
}
...
func (k Keeper) UpdateReputation(ctx sdk.Context, address string, delta int64) {
	...
	params := k.GetParams(ctx)
	newRep := int64(messaging.Reputation) + delta
	if newRep < 0 {
		newRep = 0
	}
	...
	messaging.Reputation = uint64(newRep)
	k.SetMessaging(ctx, messaging)
}
...
const walletKVPrefix = "MessagingWallet/"
