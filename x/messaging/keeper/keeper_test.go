...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.Messagings) != 0 {
		t.Errorf("expected 0 messagings in default genesis, got %d", len(gs.Messagings))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	messagingKey := types.KeyMessaging(addr)
	if string(messagingKey) != types.MessagingKeyPrefix+addr {
		t.Errorf("KeyMessaging mismatch: %s", messagingKey)
	}
