...
// ProcessL1Reputation is called at epoch end. It evaluates all messagings and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		addr := messaging.Address
		if k.shouldFreezeMessagingReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old messaging.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		if k.shouldFreezeMessagingReputationDuringDeregister(ctx, messaging.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, messaging.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if messaging.Reputation != legacy {
			messaging.Reputation = legacy
...
// dual-layer L1/L2 score stores when an messaging is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when messaging goes offline.
...
// isEpochCheater checks if an messaging was flagged as a cheater in the given epoch.
