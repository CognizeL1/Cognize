...
const (
	...
	DefaultL2BudgetPerMessagingMillis int64 = 100     // 0.1 per messaging
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporterMessaging, found := k.GetMessaging(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporterMessaging.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetMessaging, found := k.GetMessaging(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetMessaging.Status == types.MessagingStatus_MESSAGING_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	messagingCount := int64(0)
	k.IterateMessagings(ctx, func(_ types.Messaging) bool {
		messagingCount++
		return false
	})

	budgetPerMessaging := l2BudgetPerMessagingMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPerMessaging * messagingCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("messagings_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPerMessagingMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPerMessaging, types.DefaultParams().L2BudgetPerMessaging, DefaultL2BudgetPerMessagingMillis)
}
