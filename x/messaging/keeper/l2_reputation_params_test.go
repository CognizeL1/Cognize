...
func setTestMessaging(k Keeper, ctx sdk.Context, address string, registeredAt int64) {
	k.SetMessaging(ctx, types.Messaging{
		Address:          address,
		MessagingId:          address,
...
func TestSubmitL2ReportUsesConfiguredThresholdsAndWeights(t *testing.T) {
	...
	setTestMessaging(k, ctx, "reporter", ctx.BlockHeight())
...
func TestSubmitL2ReportRejectsTooNewUsingConfiguredAccountAge(t *testing.T) {
	...
	setTestMessaging(k, ctx, "reporter", 100)
	setTestMessaging(k, ctx, "target", 1)
	k.SetL1Score(ctx, "reporter", 10_000)
...
func TestBootstrapLegacyReputationSeedsDualLayerScores(t *testing.T) {
	...
	k.BootstrapLegacyReputation(ctx, "messaging", 50)

	if got := k.GetL1Score(ctx, "messaging"); got != 40_000 {
		t.Fatalf("expected l1 score 40000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "messaging"); got != 10_000 {
		t.Fatalf("expected l2 score 10000, got %d", got)
	}
	if got := k.GetTotalReputation(ctx, "messaging"); got != 50_000 {
...
func TestApplyReputationDecayRespectsZeroParams(t *testing.T) {
	...
	k.SetL1Score(ctx, "messaging", 10_000)
	k.SetL2Score(ctx, "messaging", 5_000)

	k.ApplyReputationDecay(ctx)

	if got := k.GetL1Score(ctx, "messaging"); got != 10_000 {
		t.Fatalf("expected l1 score to remain 10000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "messaging"); got != 5_000 {
