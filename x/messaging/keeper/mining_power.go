...
// ComputeAllMiningPowers iterates over all messagings and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		stake := messaging.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, messaging.Address)
