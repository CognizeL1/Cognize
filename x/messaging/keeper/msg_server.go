...
func (k msgServer) AddStake(goCtx context.Context, msg *types.MsgAddStake) (*types.MsgAddStakeResponse, error) {
	...
	return k.Keeper.AddStakeToMessaging(ctx, msg.Sender, msg.Stake, senderAddr)
}

func (k msgServer) UpdateMessaging(goCtx context.Context, msg *types.MsgUpdateMessaging) (*types.MsgUpdateMessagingResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	messaging, found := k.GetMessaging(ctx, msg.Sender)
	if !found {
	...
	if msg.Capabilities != "" {
		...
		messaging.Capabilities = caps
	}
	if msg.Model != "" {
		messaging.Model = msg.Model
	}

	k.SetMessaging(ctx, messaging)
	return &types.MsgUpdateMessagingResponse{}, nil
}
...
func (k msgServer) Heartbeat(goCtx context.Context, msg *types.MsgHeartbeat) (*types.MsgHeartbeatResponse, error) {
	...
	if ctx.BlockHeight()-messaging.LastHeartbeat < params.HeartbeatInterval {
		return nil, types.ErrHeartbeatTooFrequent
	}

	messaging.LastHeartbeat = ctx.BlockHeight()
	messaging.Status = types.MessagingStatus_MESSAGING_STATUS_ONLINE
	k.SetMessaging(ctx, messaging)
...
func (k msgServer) Deregister(goCtx context.Context, msg *types.MsgDeregister) (*types.MsgDeregisterResponse, error) {
	...
	messaging.Status = types.MessagingStatus_MESSAGING_STATUS_SUSPENDED
	k.SetMessaging(ctx, messaging)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_deregister_requested",
		sdk.NewAttribute("address", msg.Sender),
...
func (k msgServer) ReduceStake(goCtx context.Context, msg *types.MsgReduceStake) (*types.MsgReduceStakeResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := k.Keeper.ReduceStakeFromMessaging(ctx, msg.Sender, msg.Amount); err != nil {
		return nil, err
	}
...
func (k msgServer) RevealAIChallengeResponse(goCtx context.Context, msg *types.MsgRevealAIChallengeResponse) (*types.MsgRevealAIChallengeResponseResponse, error) {
	...
	if _, found := k.GetMessaging(ctx, msg.Sender); !found {
		return nil, types.ErrMessagingNotFound
	}
	if k.HasDeregisterRequest(ctx, msg.Sender) {
...
func generateMessagingID(address string, blockHeight int64) string {
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s:%d", address, blockHeight)))
	return fmt.Sprintf("messaging-%s", hex.EncodeToString(hash[:8]))
}
