...
// IncrementEpochActivity increments the transaction count for an messaging in the current epoch.
...
// GetEpochActivity returns the transaction count for an messaging in a given epoch.
...
// ProcessEpochReputation updates reputation for all messagings at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.IterateMessagings(ctx, func(messaging types.Messaging) bool {
		...
		if messaging.Status == types.MessagingStatus_MESSAGING_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: messaging.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updatedMessaging, found := k.GetMessaging(ctx, u.address)
		if found && updatedMessaging.Reputation == 0 {
			k.handleZeroReputation(ctx, updatedMessaging, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the messaging.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, messaging types.Messaging, params types.Params) {
	var burnedAtRegister sdk.Coin
	if messaging.BurnedAtRegister.Denom != "" && messaging.BurnedAtRegister.IsPositive() {
		burnedAtRegister = messaging.BurnedAtRegister
	} else {
		// Fallback for legacy messagings without snapshot
	...
	if messaging.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = messaging.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", messaging.Address, "error", err)
		} else {
			burned = true
		}
	}
	...
	if burned || !remaining.IsPositive() {
		messaging.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.SetMessaging(ctx, messaging)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"messaging_stake_burned",
		sdk.NewAttribute("address", messaging.Address),
