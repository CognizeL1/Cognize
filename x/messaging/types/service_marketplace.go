...
type MessagingService struct {
	Id              string    `json:"id"`
	MessagingAddress    string    `json:"messaging_address"`
...
func (s MessagingService) Validate() error {
	if s.Id == "" {
		return fmt.Errorf("service id is required")
	}
	if s.MessagingAddress == "" {
		return fmt.Errorf("messaging address is required")
	}
	if s.Name == "" {
...
func (s MessagingService) IsActive() bool {
...
type TaskRequest struct {
	...
	SelectedMessaging        string     `json:"selected_messaging"`
	CreatedAt            int64      `json:"created_at"`
}
