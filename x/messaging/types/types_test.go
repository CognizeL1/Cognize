...
func TestKeyMessaging(t *testing.T) {
	key := types.KeyMessaging("axon1test")
	want := types.MessagingKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("KeyMessaging = %q, want %q", key, want)
	}
}

func TestKeyMessagingDifferentAddresses(t *testing.T) {
	k1 := types.KeyMessaging("axon1aaa")
	k2 := types.KeyMessaging("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"MessagingKeyPrefix", types.MessagingKeyPrefix},
...
func TestDefaultGenesisEmptyMessagings(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.Messagings) != 0 {
		t.Errorf("default genesis should have 0 messagings, got %d", len(gs.Messagings))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "messaging" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "messaging")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErrMessagingAlreadyRegistered", types.ErrMessagingAlreadyRegistered},
		{"ErrMessagingNotFound", types.ErrMessagingNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErrMessagingSuspended", types.ErrMessagingSuspended},
