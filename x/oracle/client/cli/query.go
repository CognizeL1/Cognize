...
func CmdQueryOracle() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "oracle [address]",
		Short: "Query an oracle by address",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			res, err := queryClient.Oracle(context.Background(), &types.QueryOracleRequest{
				Address: args[0],
			})
			if err != nil {
...
func CmdQueryOracles() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "oracles",
		Short: "Query all registered oracles",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			queryClient := types.NewQueryClient(clientCtx)
			res, err := queryClient.Oracles(context.Background(), &types.QueryOraclesRequest{})
			if err != nil {
				return err
			}
...
func CmdQueryReputation() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Query an oracle's reputation score",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
