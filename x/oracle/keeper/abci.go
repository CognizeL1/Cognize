...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy oracle.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all oracles and store for validator updates
...
// checkHeartbeatTimeouts collects offline oracles first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	...
	var toOffline []offlineOracle

	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		if oracle.Status == types.OracleStatus_ORACLE_STATUS_ONLINE &&
			blockHeight-oracle.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineOracle{oracle: oracle})
		}
		return false
	})

	for _, o := range toOffline {
		o.oracle.Status = types.OracleStatus_ORACLE_STATUS_OFFLINE
		k.SetOracle(ctx, o.oracle)
		k.UpdateReputation(ctx, o.oracle.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.oracle.Address)

		k.Logger(ctx).Info("oracle went offline",
			"address", o.oracle.Address,
			"last_heartbeat", o.oracle.LastHeartbeat,
...
const jailedFlagKeyPrefix = "OracleJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// oracles and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + oracle.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, oracle.Address)
			k.UpdateReputation(ctx, oracle.Address, types.ReputationLossSlashing)
