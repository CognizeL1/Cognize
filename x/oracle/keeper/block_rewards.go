...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible oracles.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		...
		if !k.isActiveValidatorAddress(ctx, oracle.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, oracle.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(oracle.Address)
		if err != nil {
			return false
		}
...
// Oracles proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered oracles (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		...
		totalRep := k.GetTotalReputation(ctx, oracle.Address)
		if totalRep <= 0 {
			return false
		}
		...
		oracles = append(oracles, repWeight{address: oracle.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(oracles) == 0 {
		return totalAmount
	}
