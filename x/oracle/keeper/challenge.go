...
// CheaterAnswerThreshold is the minimum number of oracles submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI oracle registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		if oracle.Status == types.OracleStatus_ORACLE_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, oracle.Address) &&
			!respondents[oracle.Address] {
			k.SetAIBonus(ctx, oracle.Address, 0)
		}
		return false
	})
...
// detectCheaters flags oracles that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — oracles copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := oracle.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		oracle.StakeAmount = oracle.StakeAmount.Sub(slashCoin)
