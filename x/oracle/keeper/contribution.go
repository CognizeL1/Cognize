...
const (
	...
	// Whitepaper §9.4: Oracle 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — oracles with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — oracles registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoredOracle struct {
		address string
		score   int64
		stake   *big.Int
	}

	var oracles []scoredOracle
	totalScore := int64(0)
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		...
		if oracle.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-oracle.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, oracle)
		if score <= 0 {
			return false
		}

		stake := oracle.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		oracles = append(oracles, scoredOracle{address: oracle.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(oracles) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range oracles {
		...
		maxPerOracle := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPerOracle) > 0 {
			share.Set(maxPerOracle)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("oracles", fmt.Sprintf("%d", len(oracles))),
	))
}

// calculateContributionScore computes a weighted score for an oracle.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, oracle types.Oracle) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, oracle.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, oracle.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, oracle.Address)
	...
	if oracle.Reputation > 70 {
		score += int64(oracle.Reputation-70) * WeightHighReputation
	}
...
func contributionRewardCap(poolAmount, oracleStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || oracleStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || oracleStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, oracleStake)
