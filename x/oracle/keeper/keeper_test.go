...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.Oracles) != 0 {
		t.Errorf("expected 0 oracles in default genesis, got %d", len(gs.Oracles))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	oracleKey := types.KeyOracle(addr)
	if string(oracleKey) != types.OracleKeyPrefix+addr {
		t.Errorf("KeyOracle mismatch: %s", oracleKey)
	}
