...
const (
	...
	DefaultL2BudgetPerOracleMillis int64 = 100     // 0.1 per oracle
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporterOracle, found := k.GetOracle(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporterOracle.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetOracle, found := k.GetOracle(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetOracle.Status == types.OracleStatus_ORACLE_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	oracleCount := int64(0)
	k.IterateOracles(ctx, func(_ types.Oracle) bool {
		oracleCount++
		return false
	})

	budgetPerOracle := l2BudgetPerOracleMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPerOracle * oracleCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("oracles_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPerOracleMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPerOracle, types.DefaultParams().L2BudgetPerOracle, DefaultL2BudgetPerOracleMillis)
}
