...
// ComputeAllMiningPowers iterates over all oracles and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		stake := oracle.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, oracle.Address)
