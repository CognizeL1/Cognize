...
// IncrementEpochActivity increments the transaction count for an oracle in the current epoch.
...
// GetEpochActivity returns the transaction count for an oracle in a given epoch.
...
// ProcessEpochReputation updates reputation for all oracles at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.IterateOracles(ctx, func(oracle types.Oracle) bool {
		...
		if oracle.Status == types.OracleStatus_ORACLE_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: oracle.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updatedOracle, found := k.GetOracle(ctx, u.address)
		if found && updatedOracle.Reputation == 0 {
			k.handleZeroReputation(ctx, updatedOracle, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the oracle.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, oracle types.Oracle, params types.Params) {
	var burnedAtRegister sdk.Coin
	if oracle.BurnedAtRegister.Denom != "" && oracle.BurnedAtRegister.IsPositive() {
		burnedAtRegister = oracle.BurnedAtRegister
	} else {
		// Fallback for legacy oracles without snapshot
	...
	if oracle.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = oracle.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", oracle.Address, "error", err)
		} else {
			burned = true
		}
	}
	...
	if burned || !remaining.IsPositive() {
		oracle.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.SetOracle(ctx, oracle)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"oracle_stake_burned",
		sdk.NewAttribute("address", oracle.Address),
