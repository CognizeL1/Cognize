...
func TestKeyOracle(t *testing.T) {
	key := types.KeyOracle("axon1test")
	want := types.OracleKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("KeyOracle = %q, want %q", key, want)
	}
}

func TestKeyOracleDifferentAddresses(t *testing.T) {
	k1 := types.KeyOracle("axon1aaa")
	k2 := types.KeyOracle("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"OracleKeyPrefix", types.OracleKeyPrefix},
...
func TestDefaultGenesisEmptyOracles(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.Oracles) != 0 {
		t.Errorf("default genesis should have 0 oracles, got %d", len(gs.Oracles))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "oracle" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "oracle")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErrOracleAlreadyRegistered", types.ErrOracleAlreadyRegistered},
		{"ErrOracleNotFound", types.ErrOracleNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErrOracleSuspended", types.ErrOracleSuspended},
