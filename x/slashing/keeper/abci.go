...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy slashing.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all slashings and store for validator updates
...
// checkHeartbeatTimeouts collects offline slashings first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	blockHeight := ctx.BlockHeight()

	type offlineuslashing struct {
		slashing types.uslashing
	}

	var toOffline []offlineuslashing
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		if slashing.Status == types.uslashingStatus_SLASHING_STATUS_ONLINE &&
			blockHeight-slashing.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineuslashing{slashing: slashing})
		}
		return false
	})

	for _, o := range toOffline {
		o.slashing.Status = types.uslashingStatus_SLASHING_STATUS_OFFLINE
		k.Setuslashing(ctx, o.slashing)
		k.UpdateReputation(ctx, o.slashing.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.slashing.Address)

		k.Logger(ctx).Info("slashing went offline",
			"address", o.slashing.Address,
			"last_heartbeat", o.slashing.LastHeartbeat,
...
const jailedFlagKeyPrefix = "uslashingJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// slashings and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(slashing.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + slashing.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, slashing.Address)
			k.UpdateReputation(ctx, slashing.Address, types.ReputationLossSlashing)
