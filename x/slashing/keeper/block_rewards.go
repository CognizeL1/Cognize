...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible slashings.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		if !k.isActiveValidatorAddress(ctx, slashing.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, slashing.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(slashing.Address)
		if err != nil {
			return false
		}
...
// uslashings proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered slashings (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		totalRep := k.GetTotalReputation(ctx, slashing.Address)
		if totalRep <= 0 {
			return false
		}
		...
		slashings = append(slashings, repWeight{address: slashing.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(slashings) == 0 {
		return totalAmount
	}
