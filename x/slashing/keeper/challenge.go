...
// CheaterAnswerThreshold is the minimum number of slashings submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI slashing registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		if slashing.Status == types.uslashingStatus_SLASHING_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, slashing.Address) &&
			!respondents[slashing.Address] {
			k.SetAIBonus(ctx, slashing.Address, 0)
		}
		return false
	})
...
// detectCheaters flags slashings that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — slashings copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := slashing.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		slashing.StakeAmount = slashing.StakeAmount.Sub(slashCoin)
