...
const (
	...
	// Whitepaper §9.4: uslashing 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — slashings with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — slashings registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoreduslashing struct {
		address string
		score   int64
		stake   *big.Int
	}

	var slashings []scoreduslashing
	totalScore := int64(0)
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		if slashing.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-slashing.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, slashing)
		if score <= 0 {
			return false
		}

		stake := slashing.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		slashings = append(slashings, scoreduslashing{address: slashing.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(slashings) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range slashings {
		share := new(big.Int).Mul(poolBig, big.NewInt(a.score))
		share.Div(share, big.NewInt(totalScore))

		maxPeruslashing := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPeruslashing) > 0 {
			share.Set(maxPeruslashing)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("slashings", fmt.Sprintf("%d", len(slashings))),
	))
}

// calculateContributionScore computes a weighted score for an slashing.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, slashing types.uslashing) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, slashing.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, slashing.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, slashing.Address)
	...
	if slashing.Reputation > 70 {
		score += int64(slashing.Reputation-70) * WeightHighReputation
	}

	// Online bonus
	if slashing.Status == types.uslashingStatus_SLASHING_STATUS_ONLINE {
		score += 5
	}
...
func contributionRewardCap(poolAmount, slashingStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || slashingStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || slashingStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, slashingStake)
