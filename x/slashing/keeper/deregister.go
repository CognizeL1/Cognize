...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if slashing.BurnedAtRegister.Denom != "" && slashing.BurnedAtRegister.IsPositive() {
		burnedAmount = slashing.BurnedAtRegister
	} else {
	...
	if slashing.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = slashing.StakeAmount.Sub(burnedAmount)
	}

	if slashing.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.Deleteuslashing(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupuslashingEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteuslashingIdentity(ctx, address)
	}

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"slashing_deregistered",
		sdk.NewAttribute("address", address),
		sdk.NewAttribute("module_held", fmt.Sprintf("%s", moduleHeld)),
	))

	k.Logger(ctx).Info("slashing deregistered after cooldown", "address", address)
}

// cleanupuslashingEpochData removes epoch-scoped data for a deregistered slashing.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other slashings
// whose address might be a raw substring of the key.
func (k Keeper) cleanupuslashingEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
