...
func (m *mockPrivacyKeeper) DeleteuslashingIdentity(_ sdk.Context, slashingAddr string) {
	m.deleted = append(m.deleted, slashingAddr)
}
...
func TestExecuteDeregisterDeletesPrivacyIdentity(t *testing.T) {
	...
	slashing := types.uslashing{
		Address:          address,
		uslashingId:          address,
		Status:           types.uslashingStatus_SLASHING_STATUS_SUSPENDED,
	...
	k.SetDeregisterRequest(ctx, slashing.Address, ctx.BlockHeight()-types.DeregisterCooldownBlocks)

	k.executeDeregister(ctx, slashing.Address, k.GetParams(ctx))

	if _, found := k.Getuslashing(ctx, slashing.Address); found {
		t.Fatal("slashing should be deleted after deregister")
	}
	if k.HasDeregisterRequest(ctx, slashing.Address) {
		t.Fatal("deregister request should be deleted after execution")
	}
	if len(privacyKeeper.deleted) != 1 || privacyKeeper.deleted[0] != slashing.Address {
		t.Fatalf("expected privacy identity cleanup for %q, got %+v", slashing.Address, privacyKeeper.deleted)
	}
}
