...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.uslashings) != 0 {
		t.Errorf("expected 0 slashings in default genesis, got %d", len(gs.uslashings))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	slashingKey := types.Keyuslashing(addr)
	if string(slashingKey) != types.uslashingKeyPrefix+addr {
		t.Errorf("Keyuslashing mismatch: %s", slashingKey)
	}
