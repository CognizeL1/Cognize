...
// ProcessL1Reputation is called at epoch end. It evaluates all slashings and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		addr := slashing.Address
		if k.shouldFreezeuslashingReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old slashing.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		if k.shouldFreezeuslashingReputationDuringDeregister(ctx, slashing.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, slashing.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if slashing.Reputation != legacy {
			slashing.Reputation = legacy
...
// dual-layer L1/L2 score stores when an slashing is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when slashing goes offline.
...
// isEpochCheater checks if an slashing was flagged as a cheater in the given epoch.
