...
const (
	...
	DefaultL2BudgetPeruslashingMillis int64 = 100     // 0.1 per slashing
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporteruslashing, found := k.Getuslashing(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporteruslashing.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetuslashing, found := k.Getuslashing(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetuslashing.Status == types.uslashingStatus_SLASHING_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	slashingCount := int64(0)
	k.Iterateuslashings(ctx, func(_ types.uslashing) bool {
		slashingCount++
		return false
	})

	budgetPeruslashing := l2BudgetPeruslashingMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPeruslashing * slashingCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("slashings_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPeruslashingMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPeruslashing, types.DefaultParams().L2BudgetPeruslashing, DefaultL2BudgetPeruslashingMillis)
}
