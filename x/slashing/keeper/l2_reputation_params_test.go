...
func setTestuslashing(k Keeper, ctx sdk.Context, address string, registeredAt int64) {
	k.Setuslashing(ctx, types.uslashing{
		Address:          address,
		uslashingId:          address,
...
func TestSubmitL2ReportUsesConfiguredThresholdsAndWeights(t *testing.T) {
	...
	setTestuslashing(k, ctx, "reporter", ctx.BlockHeight())
...
func TestSubmitL2ReportRejectsTooNewUsingConfiguredAccountAge(t *testing.T) {
	...
	setTestuslashing(k, ctx, "reporter", 100)
	setTestuslashing(k, ctx, "target", 1)
	k.SetL1Score(ctx, "reporter", 10_000)
...
func TestBootstrapLegacyReputationSeedsDualLayerScores(t *testing.T) {
	...
	k.BootstrapLegacyReputation(ctx, "slashing", 50)

	if got := k.GetL1Score(ctx, "slashing"); got != 40_000 {
		t.Fatalf("expected l1 score 40000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "slashing"); got != 10_000 {
		t.Fatalf("expected l2 score 10000, got %d", got)
	}
	if got := k.GetTotalReputation(ctx, "slashing"); got != 50_000 {
...
func TestApplyReputationDecayRespectsZeroParams(t *testing.T) {
	...
	k.SetL1Score(ctx, "slashing", 10_000)
	k.SetL2Score(ctx, "slashing", 5_000)

	k.ApplyReputationDecay(ctx)

	if got := k.GetL1Score(ctx, "slashing"); got != 10_000 {
		t.Fatalf("expected l1 score to remain 10000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "slashing"); got != 5_000 {
