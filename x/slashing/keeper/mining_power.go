...
// ComputeAllMiningPowers iterates over all slashings and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		stake := slashing.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, slashing.Address)
