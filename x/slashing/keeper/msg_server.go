...
func (k msgServer) AddStake(goCtx context.Context, msg *types.MsgAddStake) (*types.MsgAddStakeResponse, error) {
	...
	return k.Keeper.AddStakeTouslashing(ctx, msg.Sender, msg.Stake, senderAddr)
}

func (k msgServer) Updateuslashing(goCtx context.Context, msg *types.MsgUpdateuslashing) (*types.MsgUpdateuslashingResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	slashing, found := k.Getuslashing(ctx, msg.Sender)
	if !found {
	...
	if msg.Capabilities != "" {
		...
		slashing.Capabilities = caps
	}
	if msg.Model != "" {
		slashing.Model = msg.Model
	}

	k.Setuslashing(ctx, slashing)
	return &types.MsgUpdateuslashingResponse{}, nil
}
...
func (k msgServer) Heartbeat(goCtx context.Context, msg *types.MsgHeartbeat) (*types.MsgHeartbeatResponse, error) {
	...
	if ctx.BlockHeight()-slashing.LastHeartbeat < params.HeartbeatInterval {
		return nil, types.ErrHeartbeatTooFrequent
	}

	slashing.LastHeartbeat = ctx.BlockHeight()
	slashing.Status = types.uslashingStatus_SLASHING_STATUS_ONLINE
	k.Setuslashing(ctx, slashing)
...
func (k msgServer) ReduceStake(goCtx context.Context, msg *types.MsgReduceStake) (*types.MsgReduceStakeResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := k.Keeper.ReduceStakeFromuslashing(ctx, msg.Sender, msg.Amount); err != nil {
		return nil, err
	}
...
func (k msgServer) RevealAIChallengeResponse(goCtx context.Context, msg *types.MsgRevealAIChallengeResponse) (*types.MsgRevealAIChallengeResponseResponse, error) {
	...
	if _, found := k.Getuslashing(ctx, msg.Sender); !found {
		return nil, types.ErruslashingNotFound
	}
	if k.HasDeregisterRequest(ctx, msg.Sender) {
...
func generateuslashingID(address string, blockHeight int64) string {
	hash := sha256.Sum256([]byte(fmt.Sprintf("%s:%d", address, blockHeight)))
	return fmt.Sprintf("slashing-%s", hex.EncodeToString(hash[:8]))
}
