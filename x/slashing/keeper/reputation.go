...
// IncrementEpochActivity increments the transaction count for an slashing in the current epoch.
...
// GetEpochActivity returns the transaction count for an slashing in a given epoch.
...
// ProcessEpochReputation updates reputation for all slashings at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		if slashing.Status == types.uslashingStatus_SLASHING_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: slashing.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updateduslashing, found := k.Getuslashing(ctx, u.address)
		if found && updateduslashing.Reputation == 0 {
			k.handleZeroReputation(ctx, updateduslashing, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the slashing.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, slashing types.uslashing, params types.Params) {
	var burnedAtRegister sdk.Coin
	if slashing.BurnedAtRegister.Denom != "" && slashing.BurnedAtRegister.IsPositive() {
		burnedAtRegister = slashing.BurnedAtRegister
	} else {
		// Fallback for legacy slashings without snapshot
	...
	if slashing.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = slashing.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", slashing.Address, "error", err)
		} else {
			burned = true
		}
	}

	slashing.Status = types.uslashingStatus_SLASHING_STATUS_SUSPENDED
	if burned || !remaining.IsPositive() {
		slashing.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.Setuslashing(ctx, slashing)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"slashing_stake_burned",
		sdk.NewAttribute("address", slashing.Address),
