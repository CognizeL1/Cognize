...
// DistributeEpochRewards distributes contribution rewards to online slashings
...
func (k Keeper) DistributeEpochRewards(ctx sdk.Context, epoch uint64) {
	...
	type weighteduslashing struct {
		address string
		weight  *big.Int
	}

	var slashings []weighteduslashing
	...
	k.Iterateuslashings(ctx, func(slashing types.uslashing) bool {
		...
		stakeAmount := slashing.StakeAmount.Amount.BigInt()

		repBonus := reputationBonusPercent(slashing.Reputation)
		aiBonus := k.GetAIBonus(ctx, slashing.Address)
		...
		slashings = append(slashings, weighteduslashing{
	...
	if totalWeight.Sign() <= 0 || len(slashings) == 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("slashings_count", fmt.Sprintf("%d", len(slashings))),
	))
}
