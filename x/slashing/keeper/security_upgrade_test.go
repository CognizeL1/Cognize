...
func TestSubmitL2ReportUsesFullWeightOnlyForIndexedEvidence(t *testing.T) {
	...
	setTestuslashing(k, ctx, "reporter", 1)
	setTestuslashing(k, ctx, "target", 1)
...
func TestSubmitL2ReportFallsBackWhenEvidenceMissingOrInvalid(t *testing.T) {
	...
	setTestuslashing(k, ctx, "reporter1", 1)
	setTestuslashing(k, ctx, "target1", 1)
	...
	setTestuslashing(k, ctx, "reporter2", 1)
	setTestuslashing(k, ctx, "target2", 1)
...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "slashing1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "slashing1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "slashing1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "slashing1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "slashing1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "slashing1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "slashing1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "slashing1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "slashing1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "slashing1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("slashing1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("slashing1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("slashing1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("slashing1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("slashing1", 2)) || !store.Has(dailyRegisterKey("slashing1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("slashing1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("slashing1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 slashings × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "slashing" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("slashing0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, slashing.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, slashing.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, slashing.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, slashing)
	if score != 801105 {
...
func TestEvaluateEpochChallengesScoresCorrectlyBeforeUpgrade(t *testing.T) {
	...
	// Register the validator as slashing
	setTestuslashing(k, ctx, wrongResp.ValidatorAddress, 1)
...
func TestHistoricalReplayConsistency(t *testing.T) {
	...
	setTestuslashing(k, ctx, resp1.ValidatorAddress, 1)
	...
	setTestuslashing(k, ctx, resp2.ValidatorAddress, 1)
