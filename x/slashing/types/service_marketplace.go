...
type uslashingService struct {
	Id              string    `json:"id"`
	uslashingAddress    string    `json:"slashing_address"`
...
func (s uslashingService) Validate() error {
	if s.Id == "" {
		return fmt.Errorf("service id is required")
	}
	if s.uslashingAddress == "" {
		return fmt.Errorf("slashing address is required")
	}
	if s.Name == "" {
...
func (s uslashingService) IsActive() bool {
...
type TaskRequest struct {
	...
	Selecteduslashing        string     `json:"selected_slashing"`
	CreatedAt            int64      `json:"created_at"`
}
