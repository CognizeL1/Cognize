...
func TestKeyuslashing(t *testing.T) {
	key := types.Keyuslashing("axon1test")
	want := types.uslashingKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("Keyuslashing = %q, want %q", key, want)
	}
}

func TestKeyuslashingDifferentAddresses(t *testing.T) {
	k1 := types.Keyuslashing("axon1aaa")
	k2 := types.Keyuslashing("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"uslashingKeyPrefix", types.uslashingKeyPrefix},
...
func TestDefaultGenesisEmptyuslashings(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.uslashings) != 0 {
		t.Errorf("default genesis should have 0 slashings, got %d", len(gs.uslashings))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "slashing" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "slashing")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErruslashingAlreadyRegistered", types.ErruslashingAlreadyRegistered},
		{"ErruslashingNotFound", types.ErruslashingNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErruslashingSuspended", types.ErruslashingSuspended},
