...
func CmdRegister() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Register as an AI state",
		Long:  "Register as an AI state on the Axon network.\nExample: axond tx state register \"coding,reasoning\" \"gpt-4\" 100000000000000000000aaxon",
...
func CmdAddStake() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Add stake to an existing AI state",
		Long:  "Increase the stake of an already-registered AI state.\nExample: axond tx state add-stake 50000000000000000000aaxon",
...
func CmdDeregister() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "deregister",
		Short: "Request to deregister as an AI state (7-day cooldown)",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
