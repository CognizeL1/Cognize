...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy state.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all states and store for validator updates
...
// checkHeartbeatTimeouts collects offline states first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	...
	for _, o := range toOffline {
		...
		k.Logger(ctx).Info("state went offline",
			"address", o.state.Address,
			"last_heartbeat", o.state.LastHeartbeat,
...
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.Iterateustates(ctx, func(state types.ustate) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(state.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + state.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, state.Address)
			k.UpdateReputation(ctx, state.Address, types.ReputationLossSlashing)
