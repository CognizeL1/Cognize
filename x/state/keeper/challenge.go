...
// CheaterAnswerThreshold is the minimum number of states submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI state registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
// detectCheaters flags states that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — states copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := state.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
