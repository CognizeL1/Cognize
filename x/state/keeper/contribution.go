...
const (
	...
	// MinReputationForReward — states with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — states registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	k.Iterateustates(ctx, func(state types.ustate) bool {
		...
		if state.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-state.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, state)
		if score <= 0 {
			return false
		}

		stake := state.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}
	...
	if totalScore <= 0 || len(states) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("states", fmt.Sprintf("%d", len(states))),
	))
}
...
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, state types.ustate) int64 {
	...
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, state.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, state.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, state.Address)
	...
	if state.Reputation > 70 {
		score += int64(state.Reputation-70) * WeightHighReputation
	}
...
func contributionRewardCap(poolAmount, stateStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || stateStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || stateStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, stateStake)
