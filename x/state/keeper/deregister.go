...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if state.BurnedAtRegister.Denom != "" && state.BurnedAtRegister.IsPositive() {
		burnedAmount = state.BurnedAtRegister
	} else {
	...
	if state.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = state.StakeAmount.Sub(burnedAmount)
	}

	if state.Reputation == 0 && moduleHeld.IsPositive() {
