...
// ProcessL1Reputation is called at epoch end. It evaluates all states and
...
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.Iterateustates(ctx, func(state types.ustate) bool {
		...
		total := k.GetTotalReputation(ctx, state.Address)
...
// dual-layer L1/L2 score stores when an state is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when state goes offline.
...
// isEpochCheater checks if an state was flagged as a cheater in the given epoch.
