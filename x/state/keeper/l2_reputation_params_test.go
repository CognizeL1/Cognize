...
func TestBootstrapLegacyReputationSeedsDualLayerScores(t *testing.T) {
	...
	k.BootstrapLegacyReputation(ctx, "state", 50)

	if got := k.GetL1Score(ctx, "state"); got != 40_000 {
		t.Fatalf("expected l1 score 40000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "state"); got != 10_000 {
		t.Fatalf("expected l2 score 10000, got %d", got)
	}
	if got := k.GetTotalReputation(ctx, "state"); got != 50_000 {
...
func TestApplyReputationDecayRespectsZeroParams(t *testing.T) {
	...
	k.SetL1Score(ctx, "state", 10_000)
	k.SetL2Score(ctx, "state", 5_000)

	k.ApplyReputationDecay(ctx)

	if got := k.GetL1Score(ctx, "state"); got != 10_000 {
		t.Fatalf("expected l1 score to remain 10000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "state"); got != 5_000 {
