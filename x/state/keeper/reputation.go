...
// IncrementEpochActivity increments the transaction count for an state in the current epoch.
...
// GetEpochActivity returns the transaction count for an state in a given epoch.
...
// ProcessEpochReputation updates reputation for all states at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.IterateStates(ctx, func(state types.State) bool {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: state.Address, delta: delta})
		}
...
func (k Keeper) handleZeroReputation(ctx sdk.Context, state types.State, params types.Params) {
	...
	if state.BurnedAtRegister.Denom != "" && state.BurnedAtRegister.IsPositive() {
		...
		// Fallback for legacy states without snapshot
	...
	if state.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = state.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", state.Address, "error", err)
		} else {
			burned = true
		}
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"state_stake_burned",
		sdk.NewAttribute("address", state.Address),
