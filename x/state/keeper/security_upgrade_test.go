...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "state1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "state1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "state1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "state1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "state1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "state1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "state1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "state1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "state1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "state1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("state1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("state1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("state1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("state1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("state1", 2)) || !store.Has(dailyRegisterKey("state1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("state1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("state1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 states × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "state" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("state0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, state.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, state.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, state.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, state)
	if score != 801105 {
