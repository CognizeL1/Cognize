...
func (k Keeper) onEpochEnd(ctx sdk.Context) {
	...
	// Sync legacy verify.Reputation field from L1+L2
	...
	// M1: Recompute mining powers for all verifys and store for validator updates
...
// checkHeartbeatTimeouts collects offline verifys first, then applies changes.
func (k Keeper) checkHeartbeatTimeouts(ctx sdk.Context, params types.Params) {
	blockHeight := ctx.BlockHeight()

	type offlineVerify struct {
		verify types.Verify
	}

	var toOffline []offlineVerify

	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		if verify.Status == types.VerifyStatus_VERIFY_STATUS_ONLINE &&
			blockHeight-verify.LastHeartbeat > params.HeartbeatTimeout {
			toOffline = append(toOffline, offlineVerify{verify: verify})
		}
		return false
	})

	for _, o := range toOffline {
		o.verify.Status = types.VerifyStatus_VERIFY_STATUS_OFFLINE
		k.SetVerify(ctx, o.verify)
		k.UpdateReputation(ctx, o.verify.Address, types.ReputationLossOffline)
		k.ApplyOfflinePenalty(ctx, o.verify.Address)

		k.Logger(ctx).Info("verify went offline",
			"address", o.verify.Address,
			"last_heartbeat", o.verify.LastHeartbeat,
...
const jailedFlagKeyPrefix = "VerifyJailedFlag/"

// processDoubleSignEvidence detects newly jailed validators among registered
// verifys and applies the L1 double-sign penalty exactly once per jailing event.
func (k Keeper) processDoubleSignEvidence(ctx sdk.Context) {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		accAddr, err := sdk.AccAddressFromBech32(verify.Address)
		if err != nil {
			return false
		}
		...
		store := ctx.KVStore(k.storeKey)
		flagKey := []byte(jailedFlagKeyPrefix + verify.Address)
		wasJailed := store.Has(flagKey)

		if validator.IsJailed() && !wasJailed {
			k.ApplyDoubleSignPenalty(ctx, verify.Address)
			k.UpdateReputation(ctx, verify.Address, types.ReputationLossSlashing)
