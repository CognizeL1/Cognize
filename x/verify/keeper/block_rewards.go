...
// by v2 MiningPower. Falls back to bonded staking tokens when no eligible verifys.
func (k Keeper) distributeValidatorRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		if !k.isActiveValidatorAddress(ctx, verify.Address) {
			return false
		}

		mp := k.GetMiningPower(ctx, verify.Address)
		if mp <= 0 {
			mp = 1
		}
		w := big.NewInt(mp)
		...
		addr, err := sdk.AccAddressFromBech32(verify.Address)
		if err != nil {
			return false
		}
...
// Verifys proportional to their ReputationScore. Unlike the old AI Performance Pool,
// this is open to ALL registered verifys (not just validators) and weights by reputation
...
func (k Keeper) distributeReputationRewards(ctx sdk.Context, totalAmount sdkmath.Int) sdkmath.Int {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		totalRep := k.GetTotalReputation(ctx, verify.Address)
		if totalRep <= 0 {
			return false
		}
		...
		verifys = append(verifys, repWeight{address: verify.Address, weight: repScore})
		totalWeight += repScore
		return false
	})

	if totalWeight <= 0 || len(verifys) == 0 {
		return totalAmount
	}
