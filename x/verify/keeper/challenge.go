...
// CheaterAnswerThreshold is the minimum number of verifys submitting the same
...
var challengePool = []struct {
	...
	{"What module in Axon handles AI verify registration?", "d4f0bc5a29de06b510f9aa428f1eedba926012b591fef7a518e776a7c9bd1824", "axon"},
...
func (k Keeper) EvaluateEpochChallenges(ctx sdk.Context, epoch uint64) {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		if verify.Status == types.VerifyStatus_VERIFY_STATUS_ONLINE &&
			k.isActiveValidatorAddress(ctx, verify.Address) &&
			!respondents[verify.Address] {
			k.SetAIBonus(ctx, verify.Address, 0)
		}
		return false
	})
...
// detectCheaters flags verifys that submitted identical normalized reveal data
// (the actual answer content). This catches real collusion — verifys copying each
...
func (k Keeper) penalizeCheater(ctx sdk.Context, address string) {
	...
	slashAmount := verify.StakeAmount.Amount.MulRaw(CheatPenaltyStakePercent).QuoRaw(100)
	if slashAmount.IsPositive() {
		...
		verify.StakeAmount = verify.StakeAmount.Sub(slashCoin)
