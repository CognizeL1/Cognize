...
const (
	...
	// Whitepaper §9.4: Verify 贡献奖励 35% = 350,000,000 AXON
	...
	// MinReputationForReward — verifys with rep < 20 don't participate
	MinReputationForReward uint64 = 20

	// MinRegistrationBlocks — verifys registered < 7 days (120960 blocks) don't participate
...
func (k Keeper) DistributeContributionRewards(ctx sdk.Context, epoch uint64) {
	...
	type scoredVerify struct {
		address string
		score   int64
		stake   *big.Int
	}

	var verifys []scoredVerify
	totalScore := int64(0)
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		if verify.Reputation < MinReputationForReward {
			return false
		}

		// Filter: registered >= 7 days
		if currentBlock-verify.RegisteredAt < MinRegistrationBlocks {
			return false
		}

		score := k.calculateContributionScore(ctx, epoch, verify)
		if score <= 0 {
			return false
		}

		stake := verify.StakeAmount.Amount.BigInt()
		if stake.Sign() <= 0 {
			return false
		}

		verifys = append(verifys, scoredVerify{address: verify.Address, score: score, stake: new(big.Int).Set(stake)})
		totalScore += score
		totalEligibleStake.Add(totalEligibleStake, stake)
		return false
	})

	if totalScore <= 0 || len(verifys) == 0 || totalEligibleStake.Sign() <= 0 {
		return
	}
	...
	for _, a := range verifys {
		share := new(big.Int).Mul(poolBig, big.NewInt(a.score))
		share.Div(share, big.NewInt(totalScore))

		maxPerVerify := contributionRewardCap(poolBig, a.stake, totalEligibleStake, capBps)

		if share.Cmp(maxPerVerify) > 0 {
			share.Set(maxPerVerify)
		}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("verifys", fmt.Sprintf("%d", len(verifys))),
	))
}

// calculateContributionScore computes a weighted score for an verify.
func (k Keeper) calculateContributionScore(ctx sdk.Context, epoch uint64, verify types.Verify) int64 {
	score := int64(0)

	// Contract deployments
	deploys := k.getCounter(ctx, types.KeyDeployCount(epoch, verify.Address))
	...
	calls := k.getCounter(ctx, types.KeyContractCall(epoch, verify.Address))
	...
	activity := k.GetEpochActivity(ctx, epoch, verify.Address)
	...
	if verify.Reputation > 70 {
		score += int64(verify.Reputation-70) * WeightHighReputation
	}

	// Online bonus
	if verify.Status == types.VerifyStatus_VERIFY_STATUS_ONLINE {
		score += 5
	}
...
func contributionRewardCap(poolAmount, verifyStake, totalEligibleStake *big.Int, capBps int64) *big.Int {
	if poolAmount == nil || verifyStake == nil || totalEligibleStake == nil {
		return new(big.Int)
	}
	if poolAmount.Sign() <= 0 || verifyStake.Sign() <= 0 || totalEligibleStake.Sign() <= 0 {
		return new(big.Int)
	}
	if capBps <= 0 {
		capBps = 200
	...
	capAmount.Mul(capAmount, verifyStake)
