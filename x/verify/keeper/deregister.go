...
func (k Keeper) executeDeregister(ctx sdk.Context, address string, params types.Params) {
	...
	if verify.BurnedAtRegister.Denom != "" && verify.BurnedAtRegister.IsPositive() {
		burnedAmount = verify.BurnedAtRegister
	} else {
	...
	if verify.StakeAmount.IsLT(burnedAmount) {
		moduleHeld = sdk.NewInt64Coin("aaxon", 0)
	} else {
		moduleHeld = verify.StakeAmount.Sub(burnedAmount)
	}

	if verify.Reputation == 0 && moduleHeld.IsPositive() {
	...
	k.DeleteVerify(ctx, address)
	k.DeleteDeregisterRequest(ctx, address)
	k.DeleteAIBonus(ctx, address)
	k.cleanupVerifyEpochData(ctx, address)
	if k.IsV111UpgradeActivated(ctx) && k.privacyKeeper != nil {
		k.privacyKeeper.DeleteVerifyIdentity(ctx, address)
	}
	...
	k.Logger(ctx).Info("verify deregistered after cooldown", "address", address)
}

// cleanupVerifyEpochData removes epoch-scoped data for a deregistered verify.
// Keys use the format "Prefix/<epoch_bytes>/<address>", so we match on the
// "/" + address suffix to avoid accidentally deleting data for other verifys
// whose address might be a raw substring of the key.
func (k Keeper) cleanupVerifyEpochData(ctx sdk.Context, address string) {
	store := ctx.KVStore(k.storeKey)
