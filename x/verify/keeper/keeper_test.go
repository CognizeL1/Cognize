...
func TestDefaultGenesis(t *testing.T) {
	...
	if len(gs.Verifys) != 0 {
		t.Errorf("expected 0 verifys in default genesis, got %d", len(gs.Verifys))
	}
...
func TestKeyFunctions(t *testing.T) {
	...
	verifyKey := types.KeyVerify(addr)
	if string(verifyKey) != types.VerifyKeyPrefix+addr {
		t.Errorf("KeyVerify mismatch: %s", verifyKey)
	}
