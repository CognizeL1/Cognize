...
// ProcessL1Reputation is called at epoch end. It evaluates all verifys and
...
func (k Keeper) ApplyReputationDecay(ctx sdk.Context) {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		addr := verify.Address
		if k.shouldFreezeVerifyReputationDuringDeregister(ctx, addr) {
			return false
		}
...
// SyncLegacyReputation updates the old verify.Reputation field from L1+L2 for backward compatibility.
func (k Keeper) SyncLegacyReputation(ctx sdk.Context) {
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		if k.shouldFreezeVerifyReputationDuringDeregister(ctx, verify.Address) {
			return false
		}

		total := k.GetTotalReputation(ctx, verify.Address)
		legacy := uint64(total / 1000)
		if legacy > 100 {
			legacy = 100
		}
		if verify.Reputation != legacy {
			verify.Reputation = legacy
...
// dual-layer L1/L2 score stores when an verify is first created or imported.
...
// ApplyOfflinePenalty applies immediate L1 penalty when verify goes offline.
...
// isEpochCheater checks if an verify was flagged as a cheater in the given epoch.
