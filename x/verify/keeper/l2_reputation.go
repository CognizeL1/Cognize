...
const (
	...
	DefaultL2BudgetPerVerifyMillis int64 = 100     // 0.1 per verify
...
func (k Keeper) SubmitL2Report(ctx sdk.Context, reporter, target string, score int8, evidence, reason string) error {
	...
	reporterVerify, found := k.GetVerify(ctx, reporter)
	if !found {
	...
	if ctx.BlockHeight()-reporterVerify.RegisteredAt < params.L2MinAccountAge {
		return fmt.Errorf("account too new")
	}

	targetVerify, found := k.GetVerify(ctx, target)
	if !found {
		return fmt.Errorf("target not registered")
	}
	if k.IsV111UpgradeActivated(ctx) && targetVerify.Status == types.VerifyStatus_VERIFY_STATUS_SUSPENDED {
...
func (k Keeper) SettleL2Reputation(ctx sdk.Context, epoch uint64) {
	...
	verifyCount := int64(0)
	k.IterateVerifys(ctx, func(_ types.Verify) bool {
		verifyCount++
		return false
	})

	budgetPerVerify := l2BudgetPerVerifyMillis(params)
	budgetCap := l2BudgetCapMillis(params)

	budget := budgetPerVerify * verifyCount
	if budget > budgetCap {
		budget = budgetCap
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("epoch", fmt.Sprintf("%d", epoch)),
		sdk.NewAttribute("verifys_affected", fmt.Sprintf("%d", len(rawDeltas))),
...
func l2BudgetPerVerifyMillis(params types.Params) int64 {
	return parseL2DecimalMillis(params.L2BudgetPerVerify, types.DefaultParams().L2BudgetPerVerify, DefaultL2BudgetPerVerifyMillis)
}
