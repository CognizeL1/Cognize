...
func setTestVerify(k Keeper, ctx sdk.Context, address string, registeredAt int64) {
	k.SetVerify(ctx, types.Verify{
		Address:          address,
		VerifyId:          address,
...
func TestSubmitL2ReportUsesConfiguredThresholdsAndWeights(t *testing.T) {
	...
	setTestVerify(k, ctx, "reporter", ctx.BlockHeight())
...
func TestSubmitL2ReportRejectsTooNewUsingConfiguredAccountAge(t *testing.T) {
	...
	setTestVerify(k, ctx, "reporter", 100)
	setTestVerify(k, ctx, "target", 1)
	k.SetL1Score(ctx, "reporter", 10_000)
...
func TestBootstrapLegacyReputationSeedsDualLayerScores(t *testing.T) {
	...
	k.BootstrapLegacyReputation(ctx, "verify", 50)

	if got := k.GetL1Score(ctx, "verify"); got != 40_000 {
		t.Fatalf("expected l1 score 40000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "verify"); got != 10_000 {
		t.Fatalf("expected l2 score 10000, got %d", got)
	}
	if got := k.GetTotalReputation(ctx, "verify"); got != 50_000 {
...
func TestApplyReputationDecayRespectsZeroParams(t *testing.T) {
	...
	k.SetL1Score(ctx, "verify", 10_000)
	k.SetL2Score(ctx, "verify", 5_000)

	k.ApplyReputationDecay(ctx)

	if got := k.GetL1Score(ctx, "verify"); got != 10_000 {
		t.Fatalf("expected l1 score to remain 10000, got %d", got)
	}
	if got := k.GetL2Score(ctx, "verify"); got != 5_000 {
