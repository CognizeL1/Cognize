...
// ComputeAllMiningPowers iterates over all verifys and returns normalized
...
func (k Keeper) ComputeAllMiningPowers(ctx sdk.Context) map[string]int64 {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		stake := verify.StakeAmount.Amount
		if !stake.IsPositive() {
			return false
		}

		totalRepMillis := k.GetTotalReputation(ctx, verify.Address)
