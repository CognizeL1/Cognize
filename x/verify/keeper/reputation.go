...
// IncrementEpochActivity increments the transaction count for an verify in the current epoch.
...
// GetEpochActivity returns the transaction count for an verify in a given epoch.
...
// ProcessEpochReputation updates reputation for all verifys at epoch boundaries.
...
func (k Keeper) ProcessEpochReputation(ctx sdk.Context, epoch uint64) {
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		if verify.Status == types.VerifyStatus_VERIFY_STATUS_OFFLINE {
		...
		if delta != 0 {
			updates = append(updates, reputationUpdate{address: verify.Address, delta: delta})
		}
	...
	for _, u := range updates {
		...
		updatedVerify, found := k.GetVerify(ctx, u.address)
		if found && updatedVerify.Reputation == 0 {
			k.handleZeroReputation(ctx, updatedVerify, params)
		}
	}
}

// handleZeroReputation burns remaining stake and suspends the verify.
// Uses the snapshot BurnedAtRegister instead of current params to avoid parameter-change drift.
func (k Keeper) handleZeroReputation(ctx sdk.Context, verify types.Verify, params types.Params) {
	var burnedAtRegister sdk.Coin
	if verify.BurnedAtRegister.Denom != "" && verify.BurnedAtRegister.IsPositive() {
		burnedAtRegister = verify.BurnedAtRegister
	} else {
		// Fallback for legacy verifys without snapshot
	...
	if verify.StakeAmount.IsLT(burnedAtRegister) {
		remaining = sdk.NewInt64Coin("aaxon", 0)
	} else {
		remaining = verify.StakeAmount.Sub(burnedAtRegister)
	}

	burned := false
	if remaining.IsPositive() {
		if err := k.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(remaining)); err != nil {
			k.Logger(ctx).Error("failed to burn stake for zero reputation — suspending without burn", "address", verify.Address, "error", err)
		} else {
			burned = true
		}
	}
	...
	if burned || !remaining.IsPositive() {
		verify.StakeAmount = sdk.NewInt64Coin("aaxon", 0)
	}
	k.SetVerify(ctx, verify)

	ctx.EventManager().EmitEvent(sdk.NewEvent(
		"verify_stake_burned",
		sdk.NewAttribute("address", verify.Address),
