...
// DistributeEpochRewards distributes contribution rewards to online verifys
...
func (k Keeper) DistributeEpochRewards(ctx sdk.Context, epoch uint64) {
	...
	type weightedVerify struct {
		address string
		weight  *big.Int
	}

	var verifys []weightedVerify
	...
	k.IterateVerifys(ctx, func(verify types.Verify) bool {
		...
		stakeAmount := verify.StakeAmount.Amount.BigInt()

		repBonus := reputationBonusPercent(verify.Reputation)
		aiBonus := k.GetAIBonus(ctx, verify.Address)
		...
		verifys = append(verifys, weightedVerify{
	...
	if totalWeight.Sign() <= 0 || len(verifys) == 0 {
		return
	}
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute("verifys_count", fmt.Sprintf("%d", len(verifys))),
	))
}
