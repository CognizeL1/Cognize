...
func TestSubmitL2ReportUsesFullWeightOnlyForIndexedEvidence(t *testing.T) {
	...
	setTestVerify(k, ctx, "reporter", 1)
	setTestVerify(k, ctx, "target", 1)
...
func TestSubmitL2ReportFallsBackWhenEvidenceMissingOrInvalid(t *testing.T) {
	...
	setTestVerify(k, ctx, "reporter1", 1)
	setTestVerify(k, ctx, "target1", 1)
	...
	setTestVerify(k, ctx, "reporter2", 1)
	setTestVerify(k, ctx, "target2", 1)
...
func TestCleanupOldEpochDataRemovesOnlyExpiredEpoch(t *testing.T) {
	...
	store.Set(types.KeyAIResponse(expiredEpoch, "verify1"), []byte("expired-response"))
	store.Set(types.KeyAIResponse(recentEpoch, "verify1"), []byte("recent-response"))
	store.Set(types.KeyEpochActivity(expiredEpoch, "verify1"), types.Uint64ToBytes(1))
	store.Set(types.KeyEpochActivity(recentEpoch, "verify1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(expiredEpoch, "verify1"), types.Uint64ToBytes(1))
	store.Set(types.KeyDeployCount(recentEpoch, "verify1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(expiredEpoch, "verify1"), types.Uint64ToBytes(1))
	store.Set(types.KeyContractCall(recentEpoch, "verify1"), types.Uint64ToBytes(1))

	k.cleanupOldEpochData(ctx, expiredEpoch)

	if store.Has(types.KeyChallenge(expiredEpoch)) || store.Has(types.KeyAIResponse(expiredEpoch, "verify1")) {
		t.Fatal("expired epoch keys should be deleted")
	}
	if !store.Has(types.KeyChallenge(recentEpoch)) || !store.Has(types.KeyAIResponse(recentEpoch, "verify1")) {
...
func TestCleanupOldDailyRegDataRemovesEntriesOlderThanOneDay(t *testing.T) {
	...
	store.Set(dailyRegisterKey("verify1", 1), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("verify1", 2), types.Uint64ToBytes(1))
	store.Set(dailyRegisterKey("verify1", 3), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, 3)

	if store.Has(dailyRegisterKey("verify1", 1)) {
		t.Fatal("day 1 entry should be deleted")
	}
	if !store.Has(dailyRegisterKey("verify1", 2)) || !store.Has(dailyRegisterKey("verify1", 3)) {
...
func TestCleanupOldDailyRegDataHandlesSlashInBinaryDaySuffix(t *testing.T) {
	...
	store.Set(dailyRegisterKey("verify1", trickyDay), types.Uint64ToBytes(1))

	k.cleanupOldDailyRegData(ctx, trickyDay+2)

	if store.Has(dailyRegisterKey("verify1", trickyDay)) {
...
func TestCleanupOldDailyRegDataBatchesLargeBacklog(t *testing.T) {
	...
	// Simulate 2600 verifys × 10 old days = 26000 entries
	for i := 0; i < 2600; i++ {
		addr := "verify" + strings.Repeat("0", 4-len(fmt.Sprintf("%d", i))) + fmt.Sprintf("%d", i)
	...
	for day := int64(1); day <= 10; day++ {
		if store.Has(dailyRegisterKey("verify0000", day)) {
...
func TestCalculateContributionScoreCapsLargeCounters(t *testing.T) {
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.KeyDeployCount(1, verify.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyContractCall(1, verify.Address), types.Uint64ToBytes(^uint64(0)))
	store.Set(types.KeyEpochActivity(1, verify.Address), types.Uint64ToBytes(250))

	score := k.calculateContributionScore(ctx, 1, verify)
	if score != 801105 {
...
func TestEvaluateEpochChallengesScoresCorrectlyBeforeUpgrade(t *testing.T) {
	...
	// Register the validator as verify
	setTestVerify(k, ctx, wrongResp.ValidatorAddress, 1)
...
func TestHistoricalReplayConsistency(t *testing.T) {
	...
	setTestVerify(k, ctx, resp1.ValidatorAddress, 1)
	...
	setTestVerify(k, ctx, resp2.ValidatorAddress, 1)
