...
func (s VerifyService) Validate() error {
	if s.Id == "" {
		return fmt.Errorf("service id is required")
	}
	if s.VerifyAddress == "" {
		return fmt.Errorf("verify address is required")
	}
	if s.Name == "" {
...
func (s VerifyService) IsActive() bool {
...
type TaskRequest struct {
	...
	SelectedVerify        string     `json:"selected_verify"`
	CreatedAt            int64      `json:"created_at"`
}
