...
func TestKeyVerify(t *testing.T) {
	key := types.KeyVerify("axon1test")
	want := types.VerifyKeyPrefix + "axon1test"
	if string(key) != want {
		t.Errorf("KeyVerify = %q, want %q", key, want)
	}
}

func TestKeyVerifyDifferentAddresses(t *testing.T) {
	k1 := types.KeyVerify("axon1aaa")
	k2 := types.KeyVerify("axon1bbb")
...
func TestKeyPrefixesUnique(t *testing.T) {
	prefixes := []struct {
		...
		{"VerifyKeyPrefix", types.VerifyKeyPrefix},
...
func TestDefaultGenesisEmptyVerifys(t *testing.T) {
	gs := types.DefaultGenesis()
	if len(gs.Verifys) != 0 {
		t.Errorf("default genesis should have 0 verifys, got %d", len(gs.Verifys))
	}
}
...
func TestModuleConstants(t *testing.T) {
	if types.ModuleName != "verify" {
		t.Errorf("ModuleName = %q, want %q", types.ModuleName, "verify")
	}
...
func TestErrorsNotNil(t *testing.T) {
	errors := []struct {
		...
		{"ErrVerifyAlreadyRegistered", types.ErrVerifyAlreadyRegistered},
		{"ErrVerifyNotFound", types.ErrVerifyNotFound},
		{"ErrInsufficientStake", types.ErrInsufficientStake},
		{"ErrVerifySuspended", types.ErrVerifySuspended},
